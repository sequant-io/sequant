...
- **Tests no longer hardcode an absolute "assumed absent" path.** `state-hook.test.ts` wrote a real directory when run as root, which broke `state-utils.test.ts`; absent paths now come from `os.tmpdir()` and a gate test keeps the literal out of `src`. (#1141)
