...
- **Upstream assessment: pre-flight labels, dedupe, fail past the 50-release window, stop filing fixes as deprecations.** A non-dry run checks that all six labels exist before filing anything and names the missing ones; `createAssessmentIssue` reuses an open issue with the same title; `getReleasesSince` throws when the baseline tag is not among the fetched releases instead of returning all 50; notes beginning `Fixed` (after an optional `Platform:` prefix) are no longer categorized `deprecation`. (#1179)
...
- **`sync` converges again when `node_modules` holds an older sequant (#1159).** `computeTemplateChanges` diffed `scripts/dev/*.sh` against the running CLI's bundle, but `copyTemplates` installs them from `resolveScriptsSymlinkTarget()` — `<project>/node_modules/sequant/templates/scripts` when a real install is there (#991). With an older sequant installed the two never agreed, so every `sync` exited 1 with `Version current, but N file(s) differ from bundled content` and `sync --dry-run` predicted an `overwrite:` the apply never made. The diff now compares a `scripts/dev` destination against the target it is actually installed from — the link target in symlink mode, the target file's rendered bytes in copy mode — so a link pointing anywhere else is still reported as drift (#1053).
- **Tests no longer hardcode an absolute "assumed absent" path.** `state-hook.test.ts` wrote a real directory when run as root, which broke `state-utils.test.ts`; absent paths now come from `os.tmpdir()` and a gate test keeps the literal out of `src` and `__tests__`. (#1141)
