...
- **Upstream assessment: pre-flight labels, dedupe, fail past the 50-release window, stop filing fixes as deprecations.** A non-dry run checks that all six labels exist before filing anything and names the missing ones; `createAssessmentIssue` reuses an open issue with the same title, and stops rather than filing when that lookup itself fails; `getReleasesSince` throws when the baseline tag is not among the fetched releases instead of returning all 50; notes beginning `Fixed` (after an optional `Platform:` or `[Product]` prefix) are no longer categorized `deprecation`. (#1179)
- **`.mcp.json` and client MCP config writes replace an existing symlink instead of writing through to its target (#1160).** A dotfiles-managed client config that was a link becomes a plain file.
- **`role:` model references and model-ladder rungs resolve against the `--agent` driver when `--agent` overrides `run.agent`.** They were resolved against `settings.run.agent` even when the run used a different driver, handing that driver another driver's model. A string-shorthand role (claude-code only) combined with `--agent codex` now stops the run at config time with the existing "string shorthand" error instead. (#1150)
- **The upstream-assessment workflow lands its report through a PR instead of pushing to `main`, which the #1109 ruleset rejects.** It commits `.sequant/upstream/` to `upstream/<version>` and opens a PR, skips when nothing changed or the branch already exists, and validates the version before using it as a branch name. The push and the PR use a GitHub App token (repo variable `UPSTREAM_APP_CLIENT_ID`, secret `UPSTREAM_APP_PRIVATE_KEY`) so the required checks run on the report PR; without the App the report is uploaded as a workflow artifact with a warning. New reports are staged with `git add -f`, since `**/.sequant/` in `.gitignore` had been rejecting them. (#1153)
- **`new-feature.sh` and `sequant run` now derive the same branch name for long issue titles.** The script cut the whole branch at 58 characters while `slugify()` cuts the slug at 50, so a later `sequant run` phase missed the script's branch and created a second one. The script now uses the `slugify()` rule; `--print-branch <N> <title>` prints the name without touching the network. (#1145)
- **`sync` converges again when `node_modules` holds an older sequant (#1159).** `computeTemplateChanges` diffed `scripts/dev/*.sh` against the running CLI's bundle, but `copyTemplates` installs them from `resolveScriptsSymlinkTarget()` — `<project>/node_modules/sequant/templates/scripts` when a real install is there (#991). With an older sequant installed the two never agreed, so every `sync` exited 1 with `Version current, but N file(s) differ from bundled content` and `sync --dry-run` predicted an `overwrite:` the apply never made. The diff now compares a `scripts/dev` destination against the target it is actually installed from — the link target in symlink mode, the target file's rendered bytes in copy mode — so a link pointing anywhere else is still reported as drift (#1053). `update` now leaves symlink-mode `scripts/dev` entries to `sync` — writing them as regular files had made `update` report `Updated 1 files` on every run — and prints `N scripts/dev link(s) out of date — run \`sequant sync\`` instead of a silent "up to date" when one is pending.
- **Tests no longer hardcode an absolute "assumed absent" path.** `state-hook.test.ts` wrote a real directory when run as root, which broke `state-utils.test.ts`; absent paths now come from `os.tmpdir()` and a gate test keeps the literal out of every test tree (`src`, `__tests__`, `scripts`, `bin`). (#1141)
...
- **The `evals/results` `fixture_commit` gate no longer false-fails in a shallow clone whose object database holds the fixture commits** (a claude.ai cloud checkout that fetched other branches): ancestry there is reported unevaluable whenever the clone is shallow, not only when the object is absent. The helpers moved to `scripts/evals/commit-ancestry.ts`. (#1140)
...
- **`/exec` warns that doc-gate tests are tautologies too (#1144).** A test that only reads a `SKILL.md`, fixture or reference page and asserts substrings calls no production code, so the tautology detector scores it 100% and fails its own CLI test in the full suite. The guidance: call the real thing inside the `it` — run the documented bash block, the parser the marker feeds, or `collectFiles` over the mirrored files.
...
- **The pre-PR sync merges `origin/main` into a branch that is already pushed instead of rebasing it (#1069).** Rebasing rewrote pushed SHAs and could only be landed by force-push. `rebaseBeforePR` now merges when the branch has an upstream other than the base ref (a fresh `worktree add -b … origin/main` branch tracks `origin/main` and still rebases), aborts the merge on conflict, and prints `merged origin/main` or `rebased onto origin/main`.
...
- **Golden corpus for `pre-tool.sh` (#1094).** `__tests__/fixtures/hook-corpus.jsonl` records the exit code and block reason the hook gives every command form it has been asked to judge (the existing suite, the hook's own block log, and Claude Code transcripts, secrets and home paths redacted). `__tests__/hook-corpus.integration.test.ts` replays it through all three hook copies, so a changed verdict fails until the snapshot is updated in the same PR. `npx tsx scripts/hook-corpus.ts --diff <ref>` prints only the verdict changes against a ref; `--harvest` adds new commands. The exec skill now requires listing changed verdicts under `Hook verdicts changed:` when `pre-tool.sh` is edited.
- **CI `canary` job: a downstream fixture project on every PR (#1098).** `__tests__/canary/downstream.integration.test.ts` does a real `npm install` of the previous minor release, runs its `init`, customizes the project the way real ones are (constitution, `.claude/settings.local.json` matcher group, hand-written `AGENTS.md`, tuned `.sequant/settings.json`, stale `^1.20.0` devDependency), then drives the PR's build through `sync --dry-run`, `sync`, `init --agent codex --yes` and `update`. It asserts dry-run decisions equal the files that changed, every customization survives byte-for-byte, no `{{PLACEHOLDER}}` lands under `.claude/`, `doctor` exits 0, and a real MCP `initialize` over stdio from the fixture's cwd — with the stale local `node_modules/sequant` present — reports the PR's version (the #1084 regression test). It runs in a new `canary` vitest project (`npm run test:canary`) that `npm test` excludes, so the required `test` check never depends on the npm registry.
- **`/release` publishes to the `next` dist-tag first and refuses `latest` without `--soaked` (#1098).** Step 9 now runs `npm publish --tag next`; new Step 9b prints the soak checklist and only runs `npm dist-tag add … latest` when `--soaked` was passed. Pinned in all three skill copies by `release-skill-soak.test.ts`.
...
- **The ownership gate's side (b) now registers every `writeFile(` site individually (#1106).** #1090's scan keyed sites by enclosing function and matched `await writeFile(` only, so a second write to a _new_ destination inside an already-registered function, a `void writeFile(...)`, or a writer declared as `const foo = async () => {…}` (which inherited the previous function's key) all passed silently. `src/lib/ownership-gate.ts` scans every `writeFile(` call, attributes it to its innermost named function by brace scope, and keys the registry by `file#function#destination-expression`; a test cross-checks the site count against a naive count so a scanner slip fails loudly. Test-only: the three real writers are unchanged.
...
- **`/release` lands the release commit through a PR (#1131).** The `main` ruleset (#1109) rejects direct pushes, so Step 6 now branches, opens the release PR, waits for `test` and `canary`, squash-merges, and Step 7 tags the merge on `main`.

- **A merge gate on `main`, and a mechanical proof for any "pre-existing failure" claim (#1093).** The `main` ruleset enforced only `deletion` and `non_fast_forward`, and targeted no ref at all (`conditions.ref_name.include: []`), so a PR could merge with stale or red CI — which is how #1036, #1042 and #1048 each shipped with "3 pre-existing failures, unrelated to this diff" in the body a week before those failures (#1086) stopped a gate run. `scripts/ruleset-main.sh --print` now emits the corrected payload (`~DEFAULT_BRANCH`, the existing two rules, and a `required_status_checks` rule naming the `test` check with `strict_required_status_checks_policy: true`), documented field by field in `docs/internal/graph-2026-09-guard/ruleset.md`; the owner applies it by hand in #1109, and the script has `--print` as its only mode so no agent can. `ci.yml`'s `build` job is renamed `test` and loses its single-entry matrix, because a matrixed job reports the context `build (22.x)` and a required context that never reports blocks every merge to `main` permanently. `scripts/settle-against-base.sh <test-file>` runs a named test at HEAD and again at the base commit in the same worktree with every `SEQUANT_*` var unset, prints both results as a `### Settled against base` block for the PR body, and restores the branch and any uncommitted work on every exit path — it also distinguishes "the test file does not exist at base" from "the test failed at base", which is the difference between a new test and a pre-existing failure. `/exec` now calls it for every red test outside the diff; `/qa` §2a sets `settle_evidence_status` and §7 step 4 floors an unbacked claim at `AC_MET_BUT_NOT_A_PLUS`. `CLAUDE.md` gains a `## Red main` rule — a failing `push` run on `main` is reverted first and debugged second, and the revert PR references the run — repeated in the release skill's pre-flight checks.
...
- **The plugin's MCP server no longer dies with `CONNECTION_CLOSED` when the open project has an older `sequant` in `node_modules` (#1084).** The plugin's `.mcp.json` launched `sequant serve` with `npx -y sequant@<pin> serve` and cwd = the open project; `npx` resolves packages by walking up from that cwd, so any stale, unused `sequant` devDependency there silently shadowed the pin — a pre-`serve` version fails the MCP handshake, and Claude Code surfaces only the generic `CONNECTION_CLOSED`. The fix is `scripts/mcp-launch.mjs`, a dependency-free `node:`-builtins-only script that spawns `npx` from an isolated `mkdtemp` cwd (no `package.json`/`node_modules` ancestor to shadow with) and forwards the real project path — its own `process.cwd()` at launch — to the child as `SEQUANT_PROJECT_DIR`; `sequant serve` `chdir`s there before doing anything else. Because the sequant checkout's own root `.mcp.json` is dual-use (both the dev repo's project config and, via `marketplace.json`'s `source: "./"`, the file plugin users install), a file-path launcher can't work in both contexts — `${CLAUDE_PLUGIN_ROOT}` (with or without a `:-` default) resolves against the _open project_ when Claude Code loads the file as a plain project config, not the plugin root. The shipped `.mcp.json` instead embeds the full launcher source inline via `node -e`, generated byte-for-byte from `scripts/mcp-launch.mjs` by `scripts/generate-mcp-launch-inline.mjs` and kept in sync by a test — no `${...}` placeholder appears anywhere in `command`, `args`, or `env`. `sequant doctor` gains a **Local sequant shadow** check that warns on a version-mismatched `node_modules/sequant` in cwd's ancestor chain or `$HOME`, comparing against the project's own `.mcp.json` pin when one exists (not just the running CLI's version, which can equal the shadow's version by construction when `doctor` itself is run from inside the shadowed project).
- Preserve `#` characters in stdin-heredoc commit subjects during conventional-commit validation (#1064).
- **`sequant init` on an already-initialized repo no longer overwrites `.sequant/settings.json` with defaults (#1071).** `createDefaultSettings` wrote `generateSettingsJsonc(DEFAULT_SETTINGS)` straight over the file, so `init --agent codex` on a tuned checkout reset `run.timeout` (5400 → 1800) and **deleted the entire `run.phases` block**, keeping only what the flag set — under the success banner, with no diff, no backup and no mention. It is now guarded: when the file exists and `--force` is not passed, only the keys the flags actually set are merged in (today `run.agent`), and when no flag key changes nothing is written at all, so the file stays byte-identical, JSONC comments included. The merge lands on the **raw parsed object** rather than round-tripping through `generateSettingsJsonc` (which emits neither `run.phases` nor `run.modelRoles` nor `scopeAssessment`) or `validateSettings` (whose nested schemas strip unknown keys) — either route would reproduce the same data loss. An unparseable existing file is left untouched with a warning instead of being replaced. Init now reports the decision on stdout naming the file it preserved; `--force` keeps the old overwrite as the documented escape hatch, the same single-override contract #814 and #990 established. The guard keys off `.sequant/settings.json` itself, not the `.claude/settings.json` that init's "already initialized" warning checks — a repo with tuned sequant settings and no `.claude/settings.json` was clobbered with no warning at all. #1059's `run.agent` persistence is kept, and an explicit `--agent claude-code` now switches back off a recorded driver instead of being a silent no-op. The unified preservation rule every writer consults remains a follow-up.
- **`init --agent <name>` on a tuned repo no longer strips the JSONC comments from `.sequant/settings.json` (#1100).** #1071 stopped the update path from clobbering tuned values, but still wrote the merged object back via `JSON.stringify(merged, null, 2)` — a whole-file rewrite that silently dropped every `//` comment `init --yes` had generated, including on the no-op-adjacent case where only `run.agent` actually changes. The update now edits the raw JSONC text directly: it locates the `"run": {...}` block with a string- and comment-aware brace scan (reusing the same in-string/escaped character scan `stripJsoncComments` already does per line) and replaces or inserts only the `"agent"` line in place, so every other byte — comments included — is untouched. The one exception is a file with no `"run": {...}` object or a non-string `run.agent`, which falls back to a structural rewrite that drops comments; `sequant init` never writes such a file. Also drops two non-discriminating assertions from #1071's AC-1 test that re-parsed a hardcoded constant instead of exercising the guard, and adds coverage for the blank-file branch that #1071 only exercised by hand.
...
- **`GitHubProvider.checkAuthSync()` can no longer hang on a stalled `gh auth status` (#1099).** This was the one network-reaching `execSync` call left un-timed after #1075 bounded `doctor`'s three sibling probes — `gh auth status` reaches the network, so a stalled request or a hung `gh` hung every caller (`doctor`, upstream assessment) with no output and no way out but Ctrl-C. It now passes the same 10s bound #1075 used for its network-reaching probe; `ETIMEDOUT` falls into the existing `catch → false`, so no new error handling was needed.
...
- **One declared ownership rule now governs every file `init`, `sync` and `update` write (#1090).** Six defects in nine months (#708, #814, #990, #1030/#1042, #1071, #1078) were the same bug: `computeTemplateChanges` walked the whole templates tree with every destination defaulting to overwrite, and the exceptions lived in four unrelated places (`CUSTOMIZABLE_FILES`, the `AGENTS.md` marker, the opencode skip, `init`'s unconditional `createDefaultSettings`), so each new template or new writer inherited overwrite and nobody was asked to declare otherwise. `src/lib/templates.ts` now holds one `ownershipPolicy(destinationPath)` returning `sequant-owned` (overwrite freely), `user-owned` (never touched after creation without an explicit `--force`, reported `preserved`) or `merge` (sequant's entries ensured, everything else kept, reported `merged`) — and it is the only source `sync`'s write-set classification, `copyTemplates`' write-path guard and `createDefaultSettings`' preserve-vs-overwrite guard consult. `templateDestination` reads a declared `TEMPLATE_ROUTES` table rather than a chain of hardcoded prefix tests, so the diff-path and write-path routers whose drift caused #708 and #1030 share one declaration. `sync --dry-run` now prints the policy decision per file and `sync` applies exactly that set — the #722 parity contract, restated in policy terms and pinned by a test. The guard is a **two-sided** gate test, because four of the six defects clobbered destinations that are not reachable by walking `templates/`: side (a) fails when any file under `templates/**` has no declared route or policy, side (b) fails when any `writeFile` call site in `init.ts`/`sync.ts`/`update.ts` writes a destination absent from `NON_TEMPLATE_DESTINATIONS` — including a brand-new writer, which is how this class has reopened four times out of six. `.claude/settings.json` is declared `sequant-owned` with `.claude/settings.local.json` named in the table as the extension point, per the #1078 ruling; behaviour for every existing file, `--force` included, is unchanged. The per-file report is printed once: the decision line carries the `run `sync --force` to replace` hint that #814's separate post-copy report used to add, and what remains after the copy fires only when `copyTemplates` preserved a file the decision pass did not predict — the #722 divergence, which is worth a loud line rather than a duplicate one. `.opencode/opencode.json` is declared `merge` (`writeOpencodeMcpConfig` keeps every non-sequant `mcp` entry) and the gate maps that write site to the exact path, so the rule is enforced rather than shadowed by the `.opencode/` prefix; the gate also asserts the policy _value_ for each destination whose defect was decided, since a check that merely finds some policy would pass a silent re-classification.

### Changed

- **The qa phase now treats an unchecked test-plan box that names a command as evidence that was declared but never run (#1065).** Section 6h reads the PR body's Test plan / Verification section: an unchecked `- [ ]` item quoting a command counts as unexecuted declared evidence and is weighed against the verdict, instead of being skipped as noise. The motivating case was a PR whose one unchecked box hid a live-API test annotated "not run here" that would have failed.
- **The pre-tool hook allows an anchored `env | grep '^PREFIX'`, and a red test outside the diff needs an issue number before it can be waved through (#1101).** A bare `env`/`printenv` piped into a grep whose pattern starts with `^` is a prefix-scoped listing, not an environment dump, so the env-dump guard no longer blocks it. The exec and qa skills now require a failing test in a file the diff never touched to be re-run with every `SEQUANT_*` variable unset and settled against base in the same worktree; if it still fails, it is recorded against an issue number rather than dismissed as pre-existing.
...
- **`sequant init --agent codex` provisions everything Codex needs to reach sequant's skills and guard hooks (#1059).** It creates `.agents/skills` as a **relative** symlink to `../.claude/skills` — the probe found an absolute symlink into another checkout namespaces every skill as `<repo-dir>:<name>`, breaking `$<phase>` invocation — and writes `.codex/config.toml` from a single template, wrapping the existing `.claude/hooks/pre-tool.sh` / `post-tool.sh` scripts unmodified (Codex's `PreToolUse` stdin payload is a superset of Claude Code's, pinned by a gate test that feeds the probe's recorded payload to the real hook). Because Codex loads project-layer hooks only once the project is marked trusted in the **user** config, and skips them silently otherwise, both init's output and the README state the exact `[projects."<path>"] trust_level = "trusted"` TOML. `doctor` gains matching Codex checks (binary, version floor, `CODEX_API_KEY` or `codex login status`, never printing the key), and `sequant update` preserves the symlink in both copy and symlink modes. A real directory at `.agents/skills` is never replaced, even under `--force`.
...
- **Tautology detector no longer flags subprocess-driven tests whose spawn path
  ...
  contained a contiguous `dist/`, `hooks/*.sh` or `scripts/**` substring. Two
  idiomatic constructions leave no such substring — `path.resolve(__dirname,
"cli.ts")` hides the location in a runtime global, and `join(REPO_ROOT,
"hooks", "pre-tool.sh")` splits the token across separate literals — so the
  advisory CI job reported whole integration suites as 100% tautological. Path
  expressions are now resolved to an absolute path and checked against the
  repository, bounded to executable file extensions and excluding
  `node_modules/` and temp sandboxes so the exemption does not widen into a
  blanket pass.
- Detector coverage gaps found alongside it (#956): a test block's callback body
  was located by the first `{` after `it(`, which could be a brace inside the
  title (`it("returns { kind: 'commits' }", ...)`) or an options object
...
- **`/merger` gains a Named-Set Boundary rule closing two silent-widen paths (#961)** — following #958's deferred question of whether `/merger`'s unconditional `gh pr merge` needs the same gate `/fullsolve` got, an audit found `/merger <issue-numbers>` merging the _named_ issues is itself the user's consent (unlike `/fullsolve`'s hidden side effect), so an `--auto-merge`-style flag would defeat the skill's purpose. Instead, `merger/SKILL.md` (all 3 mirrors) gains a delimited Named-Set Boundary rule: never merge or close a PR whose issue isn't in the invocation's arg list; on an out-of-named-set dependency or stack predecessor, halt and report the widened command instead of silently including it. Amends both Dependency Detection and Stacked PR Detection (#605) to defer to the new rule. Gate-tested (`merger-named-set-boundary-skill.test.ts`), mutation-verified. `/release`'s merge-adjacent behavior was also audited: no `gh pr merge` sites exist there, and its `git push origin main` sites are already checkout-lock-protected since #911 — no change needed.

- **Structured gap findings replace the lossy prose-scrape channel between `/qa`, `/loop`, and `sequant ready` (#937)** — the old channel was a single regex (`parseListSection` against `**Issues:**`/`**Gaps**`) that missed AC-table `NOT_MET` rows, §6d Adversarial Re-Read findings, and Risk Assessment items entirely; `/loop`'s own `### Required Fixes` extraction targeted a heading no QA template ever emitted. `/qa` now closes both templates with a `<!-- SEQUANT_QA_GAPS: {"findings":[...]} -->` marker (new §6j) classifying every real finding into a six-category taxonomy (`requirement_gap`/`dependency_gap`/`test_gap`/`repository_gap`/`risk_gap`/`execution_gap`) with required `evidence` and a `recommendedAction` of `fix_now`/`document`/`pause_for_human`. `parseQaSummary` (`src/lib/workflow/phase-executor.ts`) parses the marker and **unions** it with the legacy prose scrape (never replaces) so an unclassifiable finding is never silently dropped. `/loop`'s dead `### Required Fixes` extraction is replaced with marker parsing; `document`/`pause_for_human` findings are excluded from what both `/loop` (`selectFixableGaps`) and `sequant ready`'s fix loop (`ready-gate.ts`) are asked to fix — the structural fix for the #930 class of regression, where a loop chased a finding QA itself had framed as non-blocking. `ReadyGapItem` gains the taxonomy fields, and `sequant ready` now persists its final gap report (with marker) as an issue comment once the gate reaches a terminal state carrying a QA verdict, closing the terminal-scrollback hole under `ac` policy. Best-effort throughout: a failed marker parse or comment post never fails the gate or the QA pass.

- **Test tautology detector runs in CI as an advisory, measured job (#940)** — `tautology-advisory` in `ci.yml` runs the existing detector (`scripts/qa/tautology-detector-cli.ts`) on every PR's changed test files and always exits 0; findings render as `::warning` PR annotations and a job-summary table, plus a machine-readable `TAUTOLOGY_SUMMARY` line for accumulating a false-positive rate before deciding whether to enforce (Phase B, tracked as a follow-up on this issue). New CLI flags `--base`, `--advisory`, `--github` support this; `/qa`'s existing invocation is unaffected (no new flags passed).
...
- **`settings.run.mcpAllowlist` — explicit per-server opt-in for a Claude Desktop-only MCP (#936)** — #936's fix made phase agents stop reading Claude Desktop config entirely, in favor of an `.mcp.json`-based allowlist. This adds the deliberate escape hatch that fix proposed but deferred: a project can list exact `mcpServers` names in `settings.run.mcpAllowlist`, and phase agents will additionally receive those specific servers from Claude Desktop config — a name not present there is silently ignored, and project `.mcp.json` entries (plus the sequant server) always win over an allowlisted desktop entry of the same name. `getPhaseMcpServersConfig()` gains a `desktopAllowlist` option; threaded through both `ExecutionConfig` producers (`config-resolver.ts`, `ready-gate.ts` via `commands/ready.ts`) the same way #914's per-phase model/effort resolution is, so the two producers can't drift. This is a deliberate secret-exposure decision, not a convenience default — see [Allowlisting a Desktop-Only Server](docs/reference/run-command.md#allowlisting-a-desktop-only-server).
...
- **Production dependency bumps** — `@anthropic-ai/claude-agent-sdk` 0.3.218 → 0.3.234, `chalk` 5.6.2 → 6.0.0 (major; requires Node ≥22, already within sequant's `>=22.13.0` floor), `hono` 4.13.0 → 4.13.2, `@hono/node-server` 2.0.11 → 2.1.1, `open` 11.0.0 → 11.0.1 (#953). Dev dependencies also refreshed (#905), and `nanoid` patched past GHSA-2v37-7h3g-55p8 via `npm audit fix`.

### Fixed

- **Phase agents no longer inherit the user's entire Claude Desktop config (#936)** — `sequant run` phase agents previously received every MCP server configured in Claude Desktop (`claude_desktop_config.json`), including capability over-grant (email/payment/filesystem servers a phase has no business holding) and literal secrets serialized into the child process's `--mcp-config` argv (Claude Desktop configs can't use `${VAR}` references). Phase agents now get an allowlist instead: the sequant MCP server plus whatever the project's own `.mcp.json` declares — `getPhaseMcpServersConfig()` in `src/lib/mcp-config.ts`, wired at the single driver call site (`claude-code.ts`). `claude_desktop_config.json` is never read for phase execution. **Behavior change:** optional MCPs (Chrome DevTools, Context7, Sequential Thinking) configured only in Claude Desktop are no longer available to `/test`, `/exec`, `/fullsolve` phase agents — declare them in project `.mcp.json` instead (see [MCP Integrations](docs/guides/mcp-integrations.md)), or use the new `mcpAllowlist` opt-in below. `doctor`'s "MCP Servers (headless)" check now reports on the same `.mcp.json`-based source phase agents actually use, instead of Claude Desktop config.

- **`parseAcceptanceCriteria` no longer parses fenced AC-authoring examples as real ACs (#947)** — an issue body that quotes `- [ ] **AC-1:** ...` syntax inside a fenced code block (e.g. to illustrate the `Evidence:` clause format) had that example line scanned by the same per-line patterns as real ACs; because extraction dedupes on first occurrence, whichever came first — the fenced example or the real AC with the same ID — silently won and the other was dropped. Reproduced live on #938's own body, where the fenced example in its Proposal section shadowed its real AC-1. `parseAcceptanceCriteria` now tracks fence state (`computeFenceMask`, CommonMark-style: matching delimiter character, closing length ≥ opening length, unclosed fence runs to EOF) and excludes fenced lines from both the main scan and the `explicitIds` pre-scan, including heading detection — a `#`-prefixed line inside a fenced shell block no longer closes the AC section early. The fence-tracking logic now lives in shared `src/lib/markdown-fence.ts` — `scope/analyzer.ts`'s `parseNonGoals` had the identical fence-blindness bug (a fenced Non-Goals example read as a real non-goal item) and now uses the same `stripFencedLines` helper.

- **`/fullsolve` no longer merges the PR without confirmation (#958)** — Phase 5.3 ran an unconditional `gh pr merge <N> --squash` immediately after PR creation, contradicting the Auto-Progression section's own stop-list ("final summary after PR creation"). `Bash(gh pr merge:*)` is removed from the default `allowed-tools` grant in all three skill mirrors, and §5.3 now requires `--auto-merge`, `run.autoMerge: true` in `.sequant/settings.json`, or explicit user instruction before it runs — the workflow's default terminal state is PR created + final summary posted, left open for human review. **Behavior change:** a bare `/fullsolve <issue>` invocation no longer merges; pass `--auto-merge` for the previous end-to-end behavior. New `run.autoMerge` setting (default `false`) added to `RunSettingsSchema`.

- **`/loop`'s orchestrated Step 1A trusts the QA context already embedded in its invocation instead of re-fetching from GitHub (#960)** — Step 1A unconditionally fetched `gh issue view` for a `## QA Review for Issue` comment even when `ready-gate.ts`/`batch-executor.ts` had already embedded fresh QA findings directly into `/loop`'s own invocation prompt via `promptContext`. Under `sequant ready`, QA defers GitHub comment-posting to the orchestrator, so this fetch found nothing and silently fell back to whatever unrelated or stale comment happened to exist on the issue — reproduced live dogfooding #937 against #958/PR #959, where `/loop` reasoned from a stale `/fullsolve Complete` comment instead of the QA pass that had just run. `getPhasePrompt` (`src/lib/workflow/phase-executor.ts`) now wraps the appended `promptContext` in a `<!-- SEQUANT_PROMPT_CONTEXT -->` sentinel; `/loop`'s Step 1A checks its own invocation for that sentinel first and skips the GitHub fetch entirely when present, falling back to the fetch only when absent. Standalone `/loop <N>` (no `SEQUANT_ORCHESTRATOR`) is unaffected.

- **`pre-tool.sh`'s No-Changes Guard no longer strands real commits behind a false "no changes to commit" block (#963)** — the guard's `TARGET_DIR` extraction kept a `cd` target's surrounding quotes (`cd "$WT"`, `cd "/path"`) and never resolved shell variables, so a quoted or variable `cd` target failed its own directory check and silently fell through to checking **the hook process's own cwd** instead of the command's actual cwd — reproduced live dogfooding #960's `/loop` fix against a downstream project, where a complete, tested implementation (41/41 tests passing) never got committed. New `resolve_cd_target()` scans the raw command (not the quote-stripping `SEGMENTS`) for the _last_ `cd` line, resolves quoted or unquoted static literals, and fails open — skips the check entirely — on anything dynamic (a shell variable or command substitution) or nonexistent, since `git commit` itself already rejects a genuinely empty commit. The no-`cd` fallback now reads the command's actual cwd from the PreToolUse payload (`HOOK_CWD`, the same field the #901 checkout-lock guard already used) instead of the hook process's own `$PWD`. Also applied to that #901 checkout-lock guard's identical `cd`-extraction, which had the same quote/variable blind spot. Two hardening follow-ons in the same change: `resolve_cd_target()` also fails open on a backslash-bearing target (a `\` can escape a following `$`/backtick past the literal-string check, and carries shell meaning the function never resolves), and the jq-less fallback's `TOOL_INPUT` extraction is now escape-aware — the old `grep '"[^"]+"'` form truncated any command at its first embedded `\"`, so on a jq-less machine the guards saw `git commit -m \` instead of the full command; a sed capture of the full escaped run plus a single-scan awk unescaper replaces it. Fixed identically across all three hook mirrors (`templates/hooks/`, `hooks/`, `.claude/hooks/`).

- **Test-tautology detector no longer flags `describe.each`-table shell-script suites wholesale (#963)** — `collectBuildOutputVars` required `=` immediately after the variable name, so a type-annotated table declaration (`const HOOK_COPIES: Array<[label: string, path: string]> = [...]`) hid its `hooks/*.sh` path literals from the collector; and a helper spawning a `describe.each` callback _parameter_ (`(_label, hookPath) => ...`) had no link back to the table it was destructured from. Both fixed: the declaration matcher tolerates a bounded type annotation, and new `collectDescribeEachParams` adds a build-output table's callback parameter names to the build-output set. Measured on `__tests__/pre-tool-hook.integration.test.ts`: 74/74 blocks falsely flagged → 3 (the remainder trace to a third, structurally different gap — call-site argument-value tracking — left for a follow-up). The `tautology-advisory` CI job and `tautology-detector-cli.test.ts` no longer false-fail on branches touching spawn-driven shell-script suites.

- **Orchestrated `sequant run --phases qa` re-runs no longer leave a stale, contradicted verdict as the only visible comment (#964)** — `qa/SKILL.md` §9 promised that under `SEQUANT_ORCHESTRATOR`, "the orchestrator handles aggregated summary," but `batch-executor.ts` never posted anything: a qa phase that computed a fresh verdict different from the last-posted comment (e.g. after a gating CI check resolved) left the earlier, now-wrong verdict as the only externally-visible signal. `runIssueWithLogging` now posts a compact comment — verdict, AC coverage, gaps/suggestions, and a `<!-- SEQUANT_QA_VERDICT: {...} -->` marker — right after a standard (non-ready-gate) qa phase succeeds with a parseable verdict. Best-effort and non-fatal, mirroring the #937 ready-gate `postReport` contract; `sequant ready`'s own terminal-only reporting is unaffected. The sibling promises in `exec/SKILL.md` §6 and `test/SKILL.md` §3.2 — the same "orchestrator handles summary" wording, equally unbacked — are reworded to say what actually happens (no per-phase comment; progress/results surface through the run summary and PR body) rather than gaining runtime posting of their own, and §9's corrected wording is pinned by a scoped gate test (`qa-verdict-comment-skill.test.ts`).
...
- **Evidence-based effort escalation on quality-loop retries (#915)** — `.sequant/settings.json`'s `run.effortEscalation` (default `false`) and the new `sequant run`/`sequant ready` flag `--escalate-effort` raise a retried phase execution's reasoning effort one tier above its resolved base, built on #914's effort resolver. The design deliberately only escalates on _observed_ retry — an outer quality-loop iteration ≥ 2, or a `sequant ready` QA-pass ≥ 2 — never speculatively from a predicted difficulty signal (that predictive form stays out of scope until #914's dogfood metrics justify it), so the feature can only trade cost for quality, never the reverse. The ladder (`low → medium → high → xhigh → max`) always steps from the phase's _configured_ base, never cumulatively from a prior escalation — a base of `high` on the 3rd loop iteration lands on `xhigh`, not `max` — and caps at `max`. Applied per-execution at dispatch time rather than baked into a static `ExecutionConfig`, so escalating one phase's retry never leaks into a later phase in the chain; because the outer quality loop re-runs its whole phase list on every iteration rather than resuming from the specific phase that failed, every phase dispatched during a retried iteration escalates together. Wired at the two real retry sites (the outer quality loop in `batch-executor.ts`, and the `qa`/`loop` dispatches in `ready-gate.ts`'s QA-pass loop) — deliberately _not_ at `phase-executor.ts`'s cold-start/MCP-fallback/spec-retry loops, which recover transient infrastructure failures rather than task difficulty. An escalated execution prints under `--verbose` and records an `effortEscalations` entry in `.sequant/metrics.json` (a sibling array to #914's `phasePolicies`, since escalation is a per-execution value a flat per-run map can't express). See [Effort Escalation on Retries](docs/reference/run-command.md#effort-escalation-on-retries).

- **A checkout-scoped lock gives mutual exclusion on the shared working tree, which the per-issue lock never did (#901)** — the #625 lock is keyed on issue number, so two sessions working _different_ issues take different lock files and never contend. But `git checkout`, `switch`, `reset`, `rebase`, `merge` and `cherry-pick` are global to a working tree: the contended resource is the **checkout**, not the issue. Observed on 2026-08-05 in a downstream repo, where two `/fullsolve` sessions on issues #10 and #23 interleaved branch switches in one tree — both locks behaving exactly as designed, neither with anything to say about the shared tree. The new `.sequant/locks/checkout.lock` represents the tree itself, and `sequant locks checkout <acquire|release|check|clear>` manages it. **Enforcement lives in `pre-tool.sh`, not only the CLI**, because the racing actor is an agent's own `git checkout` Bash command — sequant's TypeScript mutates git almost exclusively via `git -C <worktree>`, so a lock only the CLI consulted would leave the actual incident path unguarded. A foreign session's branch-mutating git in the **main** checkout is refused (exit 2) with a message naming the holding session, its issue, its PID and how long it has held the tree, plus how to proceed: which worktree to use instead, the `git -C <worktree>` escape, and `sequant locks checkout clear --force` for a dead holder. The guard is deliberately narrow — `git -C <path>` forms, path-restore (`git checkout [<ref>] -- <path>`), non-mutating git, and linked worktrees (`.git` is a file there, not a directory) all pass through untouched. Staleness reuses the per-issue lock's exported `classifyStaleness` rather than reimplementing it, so the same-host dead-PID rule, the 24h absolute ceiling and `SEQUANT_MAX_LOCK_AGE_MS` behave identically by construction and an abandoned holder cannot wedge a checkout permanently (#856). The shell half of the guard reaches the same three-branch verdict — same-host dead PID, the per-TTL age cutoff (`SEQUANT_SKILL_LOCK_TTL_MS` for `skipPidCheck` holders, the 2h cross-host default otherwise), and the `SEQUANT_MAX_LOCK_AGE_MS` ceiling checked first and unconditionally — and where it cannot decide (unparseable timestamp, clock skew) it fails toward refusing rather than waving through, since refusing is recoverable and ignoring a live holder is not. `LockManager`'s numeric key is untouched: `checkout.lock` is non-numeric precisely so `list()` skips it, and `sequant locks list` queries it separately and renders it as its own row. Orchestrator/MCP mode is a no-op across the whole surface, hook included (#625 convention). `/fullsolve`'s Phase 0.3 text now states what the per-issue lock does and does not cover, so it no longer reads as a general concurrency guarantee, and every abort path releases both locks. (#901)

### Fixed

- **`inferVerificationMethod` matched keywords as substrings, not word boundaries (#946)** — `"ui"` matched inside `"requires"`, `"quick"`, `"guidance"`, and `"building"`; `"display"` and other single-word keywords had the same class of false positive. Both `VERIFICATION_KEYWORDS` (used by `inferVerificationMethod`) and the sibling `GATE_TEST_KEYWORDS` (used by `isGateTestEvidence`, feeding the #939 §6i mutation gate) now match single-word keywords with `\b`-anchored regexes instead of `.includes()`; multi-word phrases (`"unit test"`, `"skill gate"`, `"mutation-verified"`) are unaffected — they were already effectively self-anchoring. The gate-test keywords most likely to appear inflected in evidence prose (`fixture`, `section`, `flag`, `present`) tolerate an optional trailing "s" so anchoring doesn't lose plural/verb forms like "flags are wired". Re-running the #938/PR #945 missing-evidence measurement over the same 50-issue corpus: 30 → 15 flagged ACs (219 total), all 15 eliminated hits confirmed as `browser_test` false positives from the substring bug, zero new hits.

- **`/spec`'s "Agent Spawn Rules" table never executed — measured at 0 spawns across 169 `/spec` runs in the local transcript corpus — and is replaced with the strategy actually in use (#927)** — the table prescribed spawning 1–3 `sequant-explorer` subagents by issue-content heuristic, but every run instead used targeted inline `Read`/`Grep`, or occasionally the built-in `Explore`/`general-purpose` agents. `/spec`'s Context Gathering section now states that default directly — targeted `Read`/`Grep` against issue-named or discovered paths, escalating to a single built-in `Explore` agent only for genuinely open-ended discovery — mirrored across all 3 skill copies, with the `Agent(sequant-explorer)` allow-list entry swapped to `Agent(Explore)` so the escalation path stays usable. `sequant-explorer.md` is removed from `.claude/agents/` and `templates/agents/` (its role is fully absorbed by the built-in `Explore` type); `_shared/references/subagent-types.md` and `docs/features/declarative-agents.md` drop the corresponding rows/sections. **Known residual:** `sequant sync`/`update`'s `copyTemplates` is purely additive (`src/lib/templates.ts`) with no prune path, so a consumer project that already has `.claude/agents/sequant-explorer.md` keeps that stale, inert file after upgrading — nothing in the shipped skills references it anymore, so it has no effect beyond disk clutter. (#927)

- **`sequant run` now resolves the spec→run phase recommendation through a durable comment marker instead of only regexing the spec agent's ephemeral chat text (#921)** — `parseRecommendedWorkflow` read only the assistant's captured chat `text` blocks, never the GitHub comment the spec agent actually posts. When the spec agent wrote its plan to a temp file and posted it with `gh issue comment --body-file` (#814, 2026-07-26), chat text had no `## Recommended Workflow` section to match, parsing returned null, and the run silently fell back to label-based detection — which can never produce `testgen` or `security-review`. `sequant run` now resolves through an ordered chain — comment marker (`<!-- SEQUANT_SPEC: {"phases":[...],"qualityLoop":bool} -->`, Zod-validated against the phase registry) → comment prose (existing regex, applied to the comment instead of chat) → chat text (existing behavior) → label fallback — recorded per-issue in the run log as `specRecommendation: {source, phases, qualityLoop}` so fallback frequency is now auditable instead of invisible. The `/spec` skill's Output Template emits the marker alongside the existing `**Phases:**` prose, mirrored across all 3 skill copies. (#921)

- **A phase-restricted run that never implemented anything (e.g. `--phases spec`) no longer reports failed for lack of a PR (#920)** — the worktree and branch are provisioned unconditionally of which phases run, so a passing spec-only pass satisfied every conjunct of the PR-creation gate and `gh pr create` then failed with GitHub's correct-but-unhelpful "No commits between main and …", which #879 folds into a run failure. The gate now checks commits ahead of base (`hasExecChanges`, reusing #537's base resolution) before attempting a PR: zero commits ahead skips PR creation with a stated reason (`PR skipped — no commits ahead of <base> (no implementing phase ran)`, both logged and shown in the summary), leaving the run's success untouched. A branch that does have commits — including a qa-only resume — is unaffected. The CI `sequant:spec-only` trigger label, which maps to `["spec"]` and never sets `--no-pr`, inherits the same fix with no separate change. Also closes a metrics residual from #879: when PR creation is attempted and fails, `failureCategory` used to come back `undefined` (no phase failed) — it now records the new `pr_creation` category.

- **`resolveLocksDir` now resolves against the shared git checkout root instead of `process.cwd()`, so acquiring a lock from a subdirectory (or a linked worktree) is no longer a silent no-op (#909)** — found during adversarial validation of #906. `resolveLocksDir` resolved `.sequant/locks` against `process.cwd()`, while `pre-tool.sh`'s checkout guard reads it from the command's `git rev-parse --show-toplevel`. The two agreed only when the CLI happened to run from the repository root: `cd src && npx sequant locks checkout acquire --issue=42` wrote `src/.sequant/locks/checkout.lock`, the hook kept reading `<toplevel>/.sequant/locks/checkout.lock`, saw nothing, and waved every session through — the acquire succeeded while providing zero protection, and `locks list` from the root reported nothing held. The same divergence applied to the per-issue lock, since `status.ts`, `merge.ts`, `resume.ts`, `run-orchestrator.ts` and `locks.ts` all construct `LockManager()`/`CheckoutLock()` with no explicit `locksDir` — every one of them routes through `resolveLocksDir`, so fixing the one shared choke point fixes all of them with no call-site changes. The fix resolves via `git rev-parse --git-common-dir` (falling back to today's cwd-relative behavior outside a git repository), deliberately choosing `--git-common-dir` over the hook's `--show-toplevel`: a linked worktree's toplevel is its _own_ root, but its common-dir is the _main checkout's_ `.git` — and both lock types need the same shared location from any worktree, since the checkout lock represents "the tree" and the per-issue lock exists specifically to catch collisions between sessions that are each working in a different worktree of the same repo. The `--path-format=absolute` modifier from the issue's own fix sketch is deliberately **not** used — that flag needs Git >=2.31 (2021), and on an older git it would fail into the same `catch` as "not a repository," silently re-opening this exact bug with no signal that the cause was an old toolchain. Plain `--git-common-dir` (Git >=2.5, 2015) is resolved by hand instead via `resolve(cwd, out)`, which is a no-op when git already returns an absolute path (the linked-worktree case) and correctly anchors a relative one (the main-checkout case) to `cwd`. `SEQUANT_LOCKS_DIR` and an explicit `locksDir` option keep top priority, unchanged. Verified end-to-end, not just at the unit level: a new integration test drives the real built CLI to acquire the checkout lock from a subdirectory with no `SEQUANT_LOCKS_DIR` override, then runs the real `pre-tool.sh` hook against it — reproducing the bug report's exact scenario and confirming the hook now blocks a foreign session where before it silently waved every session through. (#909)

- **Stealing a stale lock is now a compare-and-swap, so two sessions can no longer both win it (#908)** — both `LockManager` and `CheckoutLock` recovered a stale lock by `unlink(lockPath)` then `openSync(…, "wx")`, two syscalls with a window between them: two sessions that both classified the same lock stale could each remove the _other's_ freshly created lock and both believe they held it — silently, and exactly the two-holder failure the lock exists to prevent. The steal now routes through one shared `stealStaleLock` helper (mirroring how both classes already share `classifyStaleness`) that takes possession atomically with `rename(lockPath → tmp)` and then verifies identity: only the stale inode it classified is discarded, and a fresh lock that appeared in the window is `link`ed back into place untouched rather than destroyed. The loser falls through to the terminal `O_CREAT|O_EXCL` create, which arbitrates the real holder. A narrow residual (a third session's create racing the restore) is documented in the helper and left for a follow-up protocol change; it never destroys a live lock and is far smaller than the original single-syscall window. Note: the plain rename-away in the issue's fix sketch is behaviorally identical to the `unlink` it replaces (a bare rename moves whatever is at the path, fresh or not) — the identity check is what makes the fix real and mutation-verifiable. (#908)

- **`new-feature.sh` no longer mutates the main checkout, closing the last bypass of the #901 checkout guard (#910)** — the guard in `pre-tool.sh` inspects only the top-level Bash command the agent runs, and `./scripts/new-feature.sh <issue>` carries no guarded git verb, so it passed through — and then the script, internally, ran `git checkout <base>` followed by `git pull` in the main checkout. A session that did not hold the checkout lock could therefore switch the holder's branch out from under it, which is exactly what #901 exists to prevent — and the guard's own refusal message recommended `new-feature.sh` as the way to proceed, so the documented escape hatch _was_ the bypass. The fix removes the mutation rather than teaching the hook to see through the script (fragile, and blind to arbitrary indirection): the worktree is now branched directly off the freshly fetched `origin/<base>` via `git worktree add -b <branch> <path> origin/<base>`, which resolves the start-point ref without ever checking out or pulling into the main tree. Because the main working tree is no longer touched, an uncommitted main checkout no longer blocks worktree creation, and the `--stash` flag — whose only purpose was to clear the way for the old `git checkout` — is now a deprecated no-op, still accepted so existing callers do not break. No hook change is needed: the operation that used to require the lock no longer exists. The same treatment covers the sibling site QA surfaced in `cleanup-worktree.sh`, whose post-cleanup "update main" step ran an unconditional `git checkout main` (plus `pull`/`rebase`) through the identical blind spot: it now updates in place only when the checkout already sits on `main`, and otherwise fast-forwards the local `main` ref via `git fetch origin main:main` — which never touches the working tree — skipping with a message when the ref is not fast-forwardable. (`release.sh` keeps its `git pull`: it refuses to run unless already on a clean `main`, so it can never switch a branch out from under a holder, and `/release` lock participation is #911's scope.) (#910)

- **`/merger` and `/release` now participate in the checkout lock instead of mutating the shared working tree unguarded (#911)** — both skills ran branch-mutating git in the **main checkout** — `/merger` a baseline `git checkout main`, an integration `git checkout -b`/`git merge`, and mid-workflow `git checkout main`; `/release` a version-bump commit/tag/push plus a guarded `git reset --soft` on rollback — while referencing the checkout lock (#901) nowhere. So they took no protection (an interleaved branch switch from another session could land `/merger`'s merge or `/release`'s commit on the wrong HEAD) and, being non-holders, were themselves refused mid-run when another session held the tree, with `clear --force` as their only documented recovery. Both now copy `/fullsolve`'s Phase 0.3 contract: acquire the checkout lock before the first guarded verb, release on **every** exit path (and _not_ on branches that continue to a later mutation — the #906 distinction), and no-op under `SEQUANT_ORCHESTRATOR`. `/merger` identifies the holder by its **first issue number**, deterministic across its multi-issue argument list. `/release` has no issue of its own and the lock proves ownership by a positive integer (both `sequant locks checkout` and the numeric-only `pre-tool.sh` guard) — the fix sketch's "express the verbs as `git -C` and it needs no lock" does not apply, since `/release` mutates the main checkout itself — so it claims the tree under a reserved sentinel issue id (`999999999`), which works with the existing CLI and hook unchanged; a symbolic `--label=release` holder would read better in `locks list` but would require changing the lock-file schema, the CLI, and the hook, and is left to a follow-up. Both edits are mirrored byte-identical across the three skill dirs (`lint:skill-sync`) and gated by per-site, mutation-verified tests plus a contention proof that drives the real `pre-tool.sh`: a foreign holder refuses each skill's first guarded command while the holding session runs it unrefused. (#911)

- **The checkout guard now recognizes the session that holds the lock, so a `/fullsolve` run is no longer blocked by its own claim (#906)** — `SEQUANT_ISSUE` was supposed to identify the holder to `pre-tool.sh`, and could never do so: `PreToolUse` runs _outside and before_ the command's shell, so nothing a skill bash block exports reaches it — not even an export prepended to the same block as the guarded command — and the one path that does export it (`sequant run`) also sets `SEQUANT_ORCHESTRATOR`, where the guard stands down. The fallback was unreachable in every real flow, so the holder was routinely refused in its own working tree, with a message naming itself as the blocking session. (Observed live while building this change.) The hook does see both the acquire and every later command of the same session, and `session_id` is the one identity that survives the shell boundary killing the acquiring PID — so it now records the session→issue binding when it observes `locks checkout acquire --issue=<N>` and consults it as the last rule in the holder check. Ordering is preserved: `sessionId` still decides when both sides carry one, then `SEQUANT_ISSUE` for a parent process that genuinely exported it, and only then the binding. Clearing happens solely on a release naming the session's _own_ issue, because a refused release must not strip the real holder's identity and lock it out of its own tree. (#906)

- **Path-restore is exempt from the checkout guard whether or not the path is quoted (#906)** — `git checkout -- "src/foo.ts"` was refused while the unquoted `git checkout -- src/foo.ts` was allowed. The cause is quoting, not shell variables: `emit_segments` drops double-quoted regions wholesale, so a quoted path left the segment as `git checkout --` with no trailing space, and the exemption pattern required `-- `. It now accepts end-of-segment as well. `git checkout --detach` still does not match, and a quoted _branch_ name (`git checkout "main"`) is still correctly blocked. (#906)

- **The test-tautology detector no longer reports whole suites of real tests as tautological (#906)** — three blind spots, each measured on `main` rather than assumed. It extracted only block-bodied **arrow-const** helpers, so a `function` declaration made every test calling it read as import-less: `checkout-lock.integration.test.ts` scored 17/19 (89%) flagged, every one false. It seeded handles only on helpers that _spawn_ the build output, so a factory like `makeLock()` returning `new CheckoutLock(...)` did not count — `checkout-lock.test.ts` at 7/20. And "production code" meant `dist/` alone, which excluded the hook scripts and `scripts/` trees this project ships and executes, including the very `pre-tool.sh` those integration tests exist to drive. All three are closed. Notably the `function` support is **not** a widened regex: a greedy return-type subpattern ran past the body and anchored on the next declaration's brace, attributing unrelated code as the helper body — it made the numbers look right for the wrong reason — so the pattern now stops at the closing paren and a deterministic walk picks the body brace, stepping over an object return-type annotation. Guard rails against over-permissiveness are explicit: a helper that reaches no production code, and one that only spawns a system binary like `git`, are both still flagged. (#906)

- **Releasing the checkout lock now requires proving you hold it, so a blocked session's release contract can no longer unlock the tree mid-run (#906)** — `CheckoutLock.release()` skipped the holder-identity check entirely whenever the lock carried `skipPidCheck`, which every `/fullsolve` lock does. Acquire and release were therefore asymmetric in exactly the two-session scenario #901 exists for: a second session's _acquire_ was correctly refused while the holder was fresh, but its _release_ succeeded and removed the holder's lock. Because every halt branch in the skill runs the release contract, the losing session finishing first was the common path, not an edge case — and the next session was then waved straight through the `pre-tool.sh` guard while the first was still mid-run. Ownership is now one ordered predicate, `isCheckoutOwner`, shared by acquire and release so the two cannot disagree about who the holder is: a foreign host is refused first and unconditionally; when both sides carry a `sessionId`, equality decides with no fall-through to anything weaker; a live process still releases its own lock by PID; and for `skipPidCheck` locks only — whose acquiring PID is dead by definition — the holder's **issue number** stands in, which is the same test `pre-tool.sh` already uses to decide whether the caller is the holder. That last rule is a courtesy check and is documented as one: the issue is readable from the lock file and `clear --force` exists, so the threat model is the accident (a _blocked_ session, which by construction carries a different issue), not an adversary. `sequant locks checkout release` accordingly takes `--issue`, and a refusal is no longer indistinguishable from a no-op — it exits non-zero naming the holder's issue and the recovery command, where "nothing was held" stays a quiet success. Two adjacent failures found while validating the fix are closed with it: a `checkout.lock` that exists but does not parse — reachable, since the file is created and written in two steps and a group-SIGKILL lands between them (#856) — was **unclearable by any command**, because `clear` read it, saw nothing, and returned `no-lock` without unlinking while the hook blocked on it forever; and all three refusal messages recommended a bare `clear`, which refuses a fresh holder, when a leaked skill lock reads fresh for its full 6h TTL. `/fullsolve`'s Phase 0.3 is corrected alongside: every release site passes `--issue`, the duplicated per-issue acquire block is gone (a literal second acquire would have halted the session against its own lock), Phase 0 is declared unconditional on resumed runs, the branch-verification gate in Phase 5.0 — the latest-firing halt path, and the only one with no release at all — now releases before exiting, and the release contract no longer names two branches that actually continue to Phase 5. `locks checkout` ships in no released version, so failing closed costs no compatibility. (#906)

- **Skill-driven `/fullsolve` now creates its worktree and `/exec` verifies the one it is handed — runs no longer execute in the main checkout, or in another repo's worktree (#899)** — three links in the skills failed together, and had since #111/#112 in January. `/fullsolve` claimed `/spec` creates the feature worktree (`/spec` never has, and says so), exported `SEQUANT_WORKTREE=../worktrees/feature/<issue>-*/` — an unexpanded glob, not a path — and `/exec` was instructed to `cd $SEQUANT_WORKTREE` and skip creation without checking anything. The `cd` failed, creation was skipped by contract, and the agent implemented in whatever directory it was already in, on whatever branch was checked out. With one session this was silent: commits landed, a PR opened, nothing errored. With two it produced interleaved `checkout`/`reset`/`commit` in a shared working tree, and unrecoverable loss of uncommitted work. A second failure mode compounded it: `../worktrees/` is derived from the parent of the git root, so **every** repository under the same parent writes into one flat namespace, and both the glob and `/exec`'s standalone lookup keyed on the issue number — which is per-repo and collides constantly. Creation was never the exposed surface (`git worktree add` onto an existing foreign path fails loudly); **resolution** was. `/fullsolve` now creates the worktree itself in a new **Phase 1.5** via `./scripts/new-feature.sh` (idempotent, so resumed sessions re-enter safely) and exports a resolved absolute path at all three export sites; `/exec` verifies before it trusts, halting on a named error rather than continuing in place; `/spec` states its worktree contract explicitly again, restoring the sentence #515's compression dropped. Both lookups now resolve through the new **`sequant worktree`** command group — `resolve <issue>` prints this repository's worktree path for an issue, `verify <path> [--issue N]` refuses one that is missing, foreign, or stale — which reads `git worktree list` in the current repository (so a sibling project's worktree structurally cannot appear) and selects on the **branch** git reports rather than the directory slug (so a worktree whose name has drifted from its branch, as `578-seo-expand-city-coverage` holding `feature/578-city-expansion-clean` already had, cannot be mis-selected). Every new failure mode is a halt: `SEQUANT_WORKTREE_NOT_FOUND`, `SEQUANT_WORKTREE_FOREIGN`, `SEQUANT_WORKTREE_ISSUE_MISMATCH`, `WORKTREE_NOT_FOUND`, `WORKTREE_AMBIGUOUS` — the bug being fixed was a fail-open one, and the fix does not replace it with a lenient guard. The `sequant run` CLI path is unaffected; it always called `ensureWorktree` before setting `SEQUANT_WORKTREE`, which is what `/exec`'s skip branch was originally written against. `../worktrees/` is deliberately **not** re-laid-out into per-project subdirectories — that would strand every existing worktree on disk, and scoping resolution closes the failure mode without it. Absorbs #900. (#899)

- **The same unverified-worktree defect is closed in every skill that touches `SEQUANT_WORKTREE`, not just `/exec` (#904)** — the pattern #899 fixed had been copied into five more places, each found by widening the search rather than trusting the first fix. `/qa` and `/loop` took the identical unguarded `cd $SEQUANT_WORKTREE`; both now run the same `sequant worktree verify` gate and halt on a named error, which matters more for `/loop` because it _writes_ — unguarded, it applied fixes and committed them in the main checkout. `/qa`, `/loop` and `/testgen` also located worktrees by globbing `../worktrees/feature/<issue>-*`, the shared-namespace bug; all three now resolve through `sequant worktree resolve`, and `/testgen` matters here because it writes test stubs into whatever tree it lands in. Three further sites keyed on a **prefix** match instead of the branch's issue number: `/merger` twice (`git worktree list | grep "feature/$ISSUE"`) and `/qa`'s Phase 0 implementation probe. `/merger`'s is the sharpest — the very next line is `git worktree remove --force`, so with `ISSUE=89` a match on `feature/899-…` force-removed an unrelated issue's worktree. `/assess`'s read-only probe is corrected for the same reason. Two new completeness invariants replace per-site pinning: one asserts every `cd $SEQUANT_WORKTREE` in **any** skill sits inside a verify-bearing guard region, the other that no skill globs `../worktrees/` or greps `git worktree list` for an issue number — so a newly-added skill that reintroduces either pattern fails the build rather than shipping. Both are scoped to spare legitimate uses: `/assess`'s `grep "^worktree"` enumerates every worktree for diff overlap and keys on nothing, so it is not flagged. (#904)

## [2.10.0] - 2026-08-05

### Added

- **Durable halt-and-resume for waitable rate-limit windows: halts write `resumeAt`, release the lock, and `sequant resume` re-enters — surviving reboots (#892)** — the #860 decision record kept `--auto-wait`'s in-process wait but named its two structural costs: the per-issue lock is held for the whole (up to 5h) wait, and a reboot kills the run while leaving a stale lock. This inverts both for the unattended case. A run halting on a waitable window (#860 classification: `five_hour`/`seven_day` with a future reset) now persists `windowHalt: { resumeAt, phase, reentries }` to issue state before exiting — `resumeAt` is the window's reset plus the same ~60 s buffer auto-wait uses — and the exit releases the per-issue lock as usual, so nothing blocks the issue while the window is closed and a reboot between halt and re-entry loses nothing. The new **`sequant resume [issues...]`** command is the re-entry: before `resumeAt` it is a clean no-op (message + exit 0, safe to invoke from cron/launchd every few minutes); after, it consumes one bounded re-entry (**2 per issue**, mirroring `AUTO_WAIT_MAX_WAITS`) and delegates to the normal `sequant run` path with `--resume` semantics, so completed phases (GitHub markers) and completed issues (#837) are skipped and locks are re-acquired normally. Exceeding the bound halts with today's labeled terminal message (exit 1) instead of ping-ponging a scheduler. Any phase success — or a non-window failure — clears the record (counter included), so `resume` never re-enters on a stale or non-waitable halt; billing failures never write one (credits are purchased, not waited out). `sequant status` and the MCP status tool surface halted issues as resumable-at rather than bare failures, and `docs/reference/halt-and-resume.md` ships the copy-pasteable cron/launchd recipe — no daemon inside sequant. `--auto-wait` is unchanged and still serves the attended case; the two compose (a spent wait budget still writes the halt record). (#892)

- **`sequant merge <issue> --watch` waits for CI checks to finish, then runs merge-check and reports — killing the "merge after green" polling loop (#818)** — the post-PR step was pure watching-a-spinner toil ("merge if green", "check CI and report the verdict"): today you either poll by hand or run merge-check too early against in-flight checks. `--watch` adds the missing _wait_ — it polls each resolved PR's `statusCheckRollup` until **every** entry reaches a terminal state (CheckRun `COMPLETED`, StatusContext `SUCCESS`/`FAILURE`/`ERROR`), then runs the normal `--check`/`--scan`/`--review` phases and prints the usual report, so the verdict is real the moment the command exits. **It never merges** — same contract as today; `--watch` only moves _when_ the existing report runs, and the human merge gate stays. The loop is **foreground only**: a plain in-process `await sleep` with a monotonic deadline, no daemon, no child process, no OS notifier — users who want a notification chain on the exit code. `--interval <seconds>` (default 30) and `--timeout <seconds>` (default 1800) are configurable — both accept whole positive seconds only and reject anything else outright, since a bare `parseInt` would turn `--timeout abc` into `NaN` (which survives a `?? default`, leaving the deadline `NaN` so the loop never gives up) and would silently read `--timeout 30m` as 30 _seconds_; the poll loop additionally clamps its own interval and deadline so it can never degrade into an unbounded, zero-delay `gh` spawn loop for a programmatic caller. A watch timeout exits with a **distinct** non-zero code (**3**), reserving 0/1/2 for the READY/NEEDS_ATTENTION/BLOCKED verdicts, and the loop always classifies a fresh poll before it can declare a timeout so a check that turns terminal on the final interval is never swallowed. Three **dispatch-block** signatures short-circuit to BLOCKED with the cause instead of polling until timeout (the trap where "0 checks after a push" means CI never started, not that CI is slow): a `CONFLICTING` mergeable state (merge conflicts block dispatch entirely), zero checks after a dispatch-blocking condition, and — reusing `src/lib/qa/infra-blocked-ci.ts` rather than reimplementing it — a uniformly-failing board whose head-SHA check-run annotations carry the runner-never-started signature (a GitHub Actions billing/spending-limit lockout, #820). The billing-annotation API call is gated on the whole board being terminal-and-failing, so healthy and partially-failing boards never pay for it. All `gh` access goes through `GitHubProvider` (the #443 consolidation) using `statusCheckRollup`, never the known-broken `--json checks` field; annotation/rollup fetch errors degrade to non-blocking so a transient failure never crashes the loop. `--watch` composes with the depth flags and `--post`, and shares a single wall-clock deadline across multiple PRs so `--timeout` bounds the whole gate. (#818)
...
- **`sequant run --qa-gate` is deprecated — it never gated anything (#795)** — the flag promised to "wait for QA pass before starting the next issue in chain", but the chain loop in `run-orchestrator.ts` broke on **any** failed link unconditionally, immediately after the `qaGate` branch that re-tested for a QA failure. The branch could only ever reach the same `break` that followed it, so `--qa-gate` produced identical behavior whether set or not, and no runtime path ever wrote the `waiting_for_qa_gate` status the feature advertised. Rather than implement the behavior the help text implied ("halt on QA failure, _continue_ past a non-QA failure"), the flag is deprecated: that continue-semantic is unsafe in chain mode specifically, because every successor rebases onto its predecessor's committed work — continuing past a failed `exec` would build each successor on an absent base, the exact failure chain mode exists to prevent. `--chain` alone already provides the gating users wanted, and for every phase rather than only `qa`. **The flag still parses** so existing scripts do not hard-error; it now prints a one-line deprecation notice **on stderr** and will be removed in a future major release. The notice is deliberately _not_ gated on `--quiet`: `--quiet` suppresses progress and version chatter rather than warnings, and the scripts most likely to still pass `--qa-gate` (CI) are also the most likely to pass `--quiet` — gating there would silence the deprecation window for exactly its target audience. Routing to stderr keeps a consuming script's piped stdout clean either way. It also **no longer requires `--chain`** — the previous `❌ --qa-gate requires --chain flag` early-return aborted the whole run, which is precisely the hard error a deprecation window must avoid. The inert branch, the misleading CLI help text, and the `QA gate: enabled (chain waits for QA pass)` run-header line are gone, and the ~75-line "QA Gate Mode" section in `docs/reference/run-command.md` — which documented a pause banner and recovery flow no code ever produced — is replaced by a migration note. The `waiting_for_qa_gate` status enum is **retained** and still handled by `status.ts`, `reconcile.ts`, and `state-cleanup.ts`, because legacy `.sequant/state.json` files can contain it and must still reconcile (#606). `qaGate` also stays in the run-log config and metrics `cliFlags` so residual usage remains measurable ahead of hard removal. The prior chain test asserted only the QA-failure-with-`--qa-gate` case, which the unconditional break satisfies either way — it passed against the inert code and had no power to detect the defect; it is replaced by a four-case matrix varying failing phase and flag independently, plus CLI integration tests pinning that the flag parses, warns, and no longer aborts. (#795)

### Changed

- **The `engines.node` floor is now `>=22.13.0`** — the declared floor of `>=22.12.0` admitted Node 22.12.x, which `inquirer@14` (`>=23.5.0 || ^22.13.0 || ^20.17.0`) rejects with an `EBADENGINE` warning (a hard failure under `engine-strict`). Anyone already on 22.13+ (or 23/24+) is unaffected; per the EOL-runtime convention this is a minor bump, not a major.
- **A `sequant run` terminated by a signal now exits `128+signum` instead of `0` (#856)** — SIGINT exits **130**, SIGTERM **143**. Previously `ShutdownManager.gracefulShutdown` printed `Interrupted. Cleanup complete.` and called `exit(0)`, so a run killed from outside reported _success_ to every caller that checks `$?` — CI steps, wrapper scripts, and `&&` chains all continued as though the run had completed. **Scripts that treat a non-zero exit as failure will now correctly see an interrupted run as a failure**; scripts that relied on Ctrl-C exiting 0 need updating. The banner also names the cause rather than describing an orderly stop, and for SIGTERM specifically points at `docs/incidents/856/`, since nothing interactive sends SIGTERM by hand. (#856)

### Fixed

- **The pre-tool hook no longer blocks creating new files inside a worktree whose path crosses a symlink** — the worktree-boundary check resolved both sides with bare `realpath`, which fails on a path that doesn't exist yet and fell back to the raw string: a Write creating a NEW file compared raw `/tmp/wt/x.ts` against the resolved `/private/tmp/wt` and was `HOOK_BLOCKED`, while edits to existing files passed. macOS hits this on every `/tmp`/`/var` worktree since both are symlinks into `/private`. The new `resolve_path_allow_missing()` walks up to the nearest existing ancestor, canonicalises it, and re-appends the missing tail so both sides normalise identically (BSD/macOS `realpath` has no `-m`). Found downstream in a dogfood project whose local fix was clobbered by every template sync — upstreamed here so synced hooks carry the cure. Mutation-verified regression tests cover all three hook copies.
- **The "run this next" commands in `sync`'s and `doctor`'s warnings now name the invocation that actually reaches the running build, instead of always saying `npx sequant`** — the hardcoded prefix is right for the documented install path (`npm install sequant` then `npx sequant …`), where npx resolves the project's `node_modules/.bin/sequant` and a bare `sequant` is not on PATH. It is wrong for a global or `npm link`ed install with no local dependency: there `npx sequant` cannot resolve locally and falls through to the **npx cache**, which can hold an older _published_ copy than the one that just printed the warning. The observed symptom is a remediation that fights itself — a drift warning whose recommended command reinstalls the very stale content that raised it, then re-arms the warning on the next run. A new `resolveCliInvocation()` picks the form per install shape (project-local dependency or npx cache → `npx sequant`; global, linked, or dev checkout → `sequant`), and the three affected messages consume it: `sync`'s outdated-skills and content-drift warnings, and `doctor`'s Skills Version check. A linked checkout resolves through its realpath (Node resolves symlinks unless `--preserve-symlinks` is set), so it matches neither the global nor the local pattern and correctly lands on the bare form. The npx-cache predicate is split out from `isLocalNodeModulesInstall` — that one deliberately excludes the cache because it answers "is this a dependency the user can `npm update`", a different question — and its Windows path is pinned by a test, since the sibling predicate checks a `\.npm\_npx\` variant _after_ normalizing separators, where it can never match.

- **`npm run build` no longer leaves the CLI entrypoint non-executable, which broke `sequant` for npm-linked development checkouts** — the build cleans `dist/bin` and re-emits `dist/bin/cli.js` via `tsc`, which writes `0644`. npm sets the executable bit on `bin` entries at install/link time, not on rebuild, so for a checkout linked into a global prefix (`npm link`, where the shim symlinks straight at `dist/bin/cli.js`) every build silently dropped the bit and the next invocation failed at the shell — `zsh: permission denied: sequant`, with no JS error and nothing pointing at the build as the cause. The build now ends in `chmod +x dist/bin/cli.js`, chained with `&&` so a failed `tsc` cannot leave a stale executable behind. Tarball installs were never affected (npm applies the bit itself), and no source is changed — this is a file-mode fix in the `build` script only.

- **Programmatic imports work for the first time — `main`/`types`/`exports` pointed at `./dist/index.js`, which the build has never emitted** — `tsc` (rootDir `.`) outputs the entry at `dist/src/index.js`, so `import { … } from "sequant"` threw `ERR_MODULE_NOT_FOUND` in every published 2.x while four docs pages advertised it; the declared `exports` map also blocked deep-path workarounds. All three fields now point at the real build output (verified against a packed-tarball install). The CLI and MCP server were never affected. Relatedly, `npm run build` now cleans `dist/src`, `dist/bin`, and `dist/dashboard` before compiling, so orphaned output from renamed sources can no longer ship silently in the tarball.
- **A fresh `sequant init` no longer reports permanent phantom drift its own remediation can't clear** — immediately after a clean init, every `doctor`/`update`/`sync` warned `5 file(s) differ from bundled content` (`.claude/mcp.json`, `.claude/relay/frame.txt`, `.claude/scripts/*.sh`), and the suggested fix, `sync --force`, printed `✔ Synced` while writing none of them — forever. `computeTemplateChanges` mapped every template to `.claude/<relpath>`, but `copyTemplates` routes scripts to `scripts/dev/`, leaves the project `.mcp.json` to `mcp-config.ts`, and never installs `templates/relay/`. A shared `templateDestination()` now keeps diff-time routing in lockstep with write-time (the destination half of #708's variable-set guarantee): fresh init reports 0 new / 0 modified, scripts diff against `scripts/dev/`, and `update` both writes to the real destinations and sets the exec bit on the `.sh` files it applies. Mutation-verified regression tests pin the routing.
- **`sequant merge` outside a git repository prints a pre-flight error instead of a raw stack trace** — the unhandled `getRepoRoot` throw dumped internal `dist/` paths; it now reports `❌ Not in a git repository.` and exits 1 like every other command's pre-flight.
- **The remaining bare `main...HEAD` consumers deferred from #878 now resolve their diff base the same way (#890)** — #878 fixed the phase-stats pipeline and the qa skill's gates, but the same stale-local-ref pattern survived at three runtime sites and in six other skills' prose. The QA cache (`qa-cache.ts`) keyed its diff hash and scoped its invalidation checks on `git diff main...HEAD`, so a lagging local `main` leaked merged-upstream content into the cache key (spurious hash-mismatch invalidations) and misattributed upstream `package.json`/`.ts` churn to the branch under review (spurious global and per-check invalidations); all five of its git calls now resolve via `resolveDiffBase`, run array-form (`execFileSync`), and honor a new injectable `QACacheOptions.cwd` (default `process.cwd()`) — which also makes the class testable against real repositories without `process.chdir`. **Existing QA caches invalidate once** on upgrade, since the ref inside the hash key changes; that is a single fresh QA run, not a regression. The per-worktree diff summaries (`worktree-manager.ts`) and the file-context list handed to file-oriented drivers like aider (`phase-executor.ts`, keeping its per-worktree `cwd`) had the same phantom-files failure — the latter materially wrong for non-claude-code backends, which would receive already-merged files as "the changeset". The commit-SHA-anchored incremental-QA diffs were already ref-correct but now also honor the injected `cwd` and array-form invocation for consistency. In skill prose, all 13 bare `main...HEAD` across `exec` (×4), `docs` (×4), `test`, `merger`, `assess` (+ its collision-detection reference), and `loop` were audited per occurrence — every one is a worktree-diff context, none intentionally local — and origin-qualified identically across all three mirrors, plus one same-root-cause `main..HEAD` commit-list sibling in `merger`. Regression-tested against real repositories with the local ref deliberately behind the remote, covering the cache hash, both invalidation scopes, and the injected-cwd incremental path. (#890)
- **Five-hour window exhaustion is no longer misclassified as terminal billing, so `--auto-wait` actually fires on subscription plans (#860)** — `isBillingFailure` classified on the presence of `overageDisabledReason: "out_of_credits"` alone, and on a subscription plan hitting the five-hour cap with overage disabled produces exactly that marker _alongside_ `rateLimitType: "five_hour"` and a live `resetsAt` — a window that reopens at a known time, not a wallet needing a top-up. Every one of the 26 real rate-limit failures captured across two consumer projects (five separate days; all committed as fixtures with provenance in `__tests__/fixtures/rate-limit-payloads-860.json`, see `docs/incidents/860/captures/`) had this shape, became a `BillingError`, and — because `shouldAutoWaitForReset` only ever waits on a `RateLimitError` — made `--auto-wait` (#804) inert on every limit event the feature was built for, with #799 then fail-fasting the run. The classification is **narrowed, not inverted**: an explicit `errorCode: "credits_required"` stays terminal even beside window evidence (it is the SDK's direct "purchase needed" signal), a missing/past `resetsAt` stays terminal, and the window vocabulary is an allowlist (`five_hour`, `seven_day*` prefix) so an unrecognized payload shape **fails closed** to today's halt rather than into an indefinite sleep — the discriminating fields (`canUserPurchaseCredits` etc.) are absent from all 26 captures, so the window shape is the proxy and its absence means terminal. The driver's `billing_error` assistant-enum downgrade gets the matching counter-exception (window evidence in the `rate_limit_event` wins over the bare enum, which carries no metadata), while event-retention semantics (`isRateLimitFailureInfo`) deliberately keep the pre-#860 markers so no failure event is dropped. A now-waitable window renders as `Rate limited — resets at HH:MM` instead of the misleading `Out of credits`. **A waiting run is also now visible without a TTY** (AC-6): the `-q` heartbeat emits an append-only line naming the wake time every 5 minutes (it was TTY-gated to a `\r` rewrite; a background wait was indistinguishable from the #856 hang), and the default-mode non-TTY renderer announces the wait once, names the wake time in its 60s `still running` heartbeat, and no longer prints a spurious `✘` failure line per 15-second wait tick (the `waiting` event fell through its start/complete/else chain — also fixed: waiting ticks no longer reset the heartbeat idle clock, which had silenced it for the whole wait). The in-process-wait vs halt-and-resume tradeoff (lock held for the wait; a reboot loses the run) is decided and recorded in `docs/investigations/auto-wait-vs-halt-resume-860.md`: keep the in-process wait now, durable halt-and-resume filed as #892. **Hardening beyond the captured shape:** an affirmatively informational rate-limit event (`status: allowed`/`allowed_warning`) is never classified waitable — a stale near-limit warning misattributed to an unrelated failure must cost an immediate halt, not a five-hour sleep (the 26 captures carry no `status` field, so absent status stays waitable); the driver's `billing_error` counter-exception is scoped to the full captured shape (window **plus** `out_of_credits`), so a pure throttle beside a billing enum keeps the #732 billing-wins rule; when a payload is terminal _only_ because its window type is unrecognized, the halt message names the rejected type (`unrecognizedWindowHint`) so Anthropic vocabulary drift is diagnosable instead of silently reading as a wallet failure; orchestrator/MCP runs emit throttled `SEQUANT_PROGRESS` waiting lines naming the wake time (each line also resets the MCP inactivity timeout that would otherwise kill a legitimate pause as "no progress", up to the transport's absolute 2h ceiling) and the wait is persisted to issue state (`autoWait.wakeAt` + paused phase), which `sequant status` and the MCP status tool now surface; and the wait machinery gained real-clock smoke tests (no injected timers) covering the tick loop, prompt abort, and ledger accounting on real `setTimeout`. (#860)
- **Phase diff stats no longer compare against a stale local base ref, and a branch that never moved off its base no longer logs the base tip as its phase commit (#878)** — worktrees are created from `origin/<base>` (`worktree-manager`), but `getGitDiffStats` diffed against the **local** `<base>` ref, which nothing in the run path updates. When local `main` lagged the remote — routine — every phase log was attributed a diff belonging to commits the run never made (observed: `sequant run 765` in matcha-maps logged 10 modified files, all from #778 which had merged **before** the run started, into both the exec and qa phase logs), and the bogus `filesModified` flowed into `run-reflect`, `merge-check` overlap/residual decisions, and `sequant logs` output. A new `resolveDiffBase` picks between `origin/<base>` and the bare ref by nearest-to-HEAD (`rev-list --count`), preferring the origin-qualified form on a tie — nearest-wins matches the worktree's actual creation point in **both** staleness directions: the stale-local-default case above _and_ a chain-mode worktree branched from a local base with unpushed commits, where blind origin-prefixing would re-create the phantom diff the other way around; when no candidate resolves (remote-less repo) it falls back to the bare ref verbatim, preserving the old behavior. `getCommitHash` gains an optional `baseRef` and returns `undefined` when HEAD has no commits past it, so an empty branch's phase log records no commit rather than the base tip masquerading as one (`7dba228` in the motivating run); the run-level start/end markers deliberately keep the bare "where is HEAD" behavior, and a git error while counting fails open so a transient failure can't erase a real hash. Regression-tested against real repositories with the local ref deliberately behind the remote. The QA phase's shell gates had the same inconsistency: all 32 bare `git diff main...HEAD` in `qa/SKILL.md` (the §7 gates included), 18 more across the qa reference docs, and the two gate scripts (`semgrep-scan.ts --changed-only`, `tautology-detector-cli.ts`) now resolve `origin/main` the same way, identically across all three skill mirrors — on the motivating run these gates scanned #778's merged code instead of the branch under review. (#878)
- **`sequant run` no longer reports success when exec leaves its work uncommitted, or when PR creation fails (#879)** — `hasExecChanges` accepted a dirty tree as proof of work (`porcelain.length > 0`), so an exec that finished with everything uncommitted "passed" — and then rebase, push, and PR creation all failed in sequence while the summary still printed `1 passed - 0 failed` (observed: `sequant run 765` in matcha-maps left ~400 lines of complete implementation dirty in the worktree and walked away reporting success — #534's empty-branch escape, reopened via a new route). A three-state `classifyExecChanges` (`commits` / `uncommitted` / `none` / `unknown`) now distinguishes the cases: exec **fails** on both `none` (the #534 case, message unchanged) and `uncommitted`, with a failure message naming each dirty path (capped at 20) and the worktree location, so the work is discoverable from the run log alone. Deliberately **no auto-commit** — sweeping up whatever the agent happened to leave behind would author a PR nobody wrote; the classifier is read-only, so the worktree and its dirty files survive the failure untouched (integration-tested against a real git worktree). `sequant ready` reports the same state as a distinct `UNCOMMITTED_ONLY` block reason (exit code 2) instead of folding it into `NO_IMPLEMENTATION`. And the final backstop #534 identified but never closed is closed: a failed `createPR` now fails the issue — `prCreationError` on the result, run-log status `failure` (held over #856's completion-time re-derivation via a forced-failure marker), counted under `failed` in the summary with the PR error as the reason — wired through a single `recordIssueCompletion` helper shared by the live orchestrator path and the batch loop, so the two completion paths cannot drift. `--no-pr` runs are unaffected, and a git error still classifies as `unknown` and fails open, so a transient git failure cannot spuriously fail an exec phase. (#879)
- **The `/exec` parallel-group marker filename is now project-scoped, so one project's parallel run can no longer redirect worktree enforcement for every concurrent Claude session on the machine (#881)** — the marker was a global constant (`/tmp/claude-parallel-group-1.marker`), and both hooks resolved the enforced worktree by globbing the whole `$TMPDIR` namespace and taking the **first** matching marker, with no project scoping. A parallel group in one project therefore hijacked worktree enforcement in every other concurrent session on the box: the sessions that lost the race saw a `HOOK_BLOCKED: File operation must be within worktree` naming a worktree from an unrelated repo (blocking legitimate edits), and — worse — the guard that exists to stop agents editing the main repo instead of the worktree was silently pointed somewhere meaningless for the duration. It presented as a transient, unreproducible glitch that cleared once the other run finished, and the dogfood testbed (sequant npm-linked into other projects) makes concurrent runs routine. The marker name now carries a per-project hash — `${TMPDIR}/claude-parallel-<project-hash>-<group-id>.marker` — so two projects running parallel groups at once cannot collide, and the readers glob only their own project-scoped prefix at all three sites (pre-tool enforcement + agent-id enumeration, post-tool agent-id enumeration). The scheme lives in one sourced helper (`parallel-marker.sh`, mirrored across all three hook dirs) that both the `/exec` skill (the writer) and the two hooks (the readers) share, so the filename and the project-root derivation cannot drift apart; the derivation uses `CLAUDE_PROJECT_DIR` (set by Claude Code in both contexts) and falls back to the git toplevel. As defense-in-depth, the marker now also stores its owning project root on line 2, and the enforcement reader ignores any marker whose stored root differs from the current project — so a stale or hand-placed marker with a colliding name cannot redirect enforcement either. (#881)
- **The tautology detector no longer scores subprocess-driven integration tests 100% tautological (#885)** — `cli.integration.test.ts` imports only node builtins and exercises production code the only way an end-to-end CLI test can: by spawning `node dist/bin/cli.js` and asserting on its output. Static import analysis can't see through the subprocess boundary, so every block scored tautological and the local `/qa` gate reported a wrong 100% verdict on any PR touching such a file — training people to ignore the detector. The detector now counts a block as calling production code when it spawns the project's `dist/` build output (via `execSync`/`spawnSync`/etc.), whether directly or through a describe/module-scope helper that does so (resolved transitively, so a test calling `expectFlagAccepted → runInUninitializedDir → spawnSync` still counts). Detection keys on **content** — a child-process spawn referencing a build-output path — never on the filename or an opt-out annotation, and it stays **per-block**: a genuinely tautological test that only asserts on locals is still reported even in a subprocess-driven file. A helper that merely mentions `dist/` in a string but never spawns is not treated as production. AC-4: the gate is confirmed a **local/agent-only advisory** run by `/qa`, inert in GitHub CI by design (shallow checkout → empty `git diff main...HEAD`); this is now documented in the QA quality-gates reference and the detector CLI header rather than made a hard CI gate. (#885)
- **A run killed mid-flight is no longer logged as a success (#856)** — `LogWriter.startIssue` seeded `status: "success"` and only `logPhase` ever revised it, so an issue whose first phase never completed was persisted as a **pass with an empty `phases[]` and `totalDurationSeconds: 0`** (observed: `run-2026-07-29T06-37-48-*.json` recording `summary.passed: 1` for a run with zero completed phases). `completeIssue` now derives status from the phase list instead of trusting that seed, `deriveIssueLogStatus([])` returns `failure` (zero completed phases is never a pass), and finalizing under a signal marks every in-flight issue with additive `aborted`/`abortReason` fields plus a run-level `abortedBy` and a `summary.aborted` sub-count. An abort forces `failure` even over a green prefix of phases — an issue still in flight when the signal arrived never reached a terminal state, so a partial run of passing phases is not a pass. All new fields are optional and the schema stays at `version: 1`, so existing logs still parse; `sequant logs` renders such a run as **ABORTED** naming the signal. Note the limit: `SIGKILL` is uncatchable, so this covers catchable signals only. (#856)
- **An abandoned lock whose PID the OS recycled no longer blocks its issue forever (#856)** — `classifyStaleness` returned "fresh" whenever a same-host PID was alive, with **no age bound at all**. A PID is only stable identity while its process lives; once recycled, an abandoned lock points at an unrelated process and reads as held indefinitely (observed: `505.lock` from 2026-05-14 still blocking #505 76 days later, alongside `708.lock` and `803.lock`). An absolute **24h ceiling** is now checked _before_ the same-host PID branch and binds regardless of host, PID liveness, or `--skip-pid-check` — 24h being ~48× the 30-minute phase timeout and 4× the skill-lock TTL, so no real run can reach it. Override with `SEQUANT_MAX_LOCK_AGE_MS`. This is also the only recovery path for a lock leaked by a SIGKILLed run, where no in-process release handler can fire. Relatedly, `--force --signal-other` now **refuses** to signal a holder past that ceiling (`stale-pid-untrusted`): the liveness probe cannot distinguish a recycled PID from a live holder — a recycled PID _is_ alive, it just belongs to someone else's program — so signalling it would SIGTERM an unrelated process on behalf of a lock nobody holds. (#856)
...
- **Worktree provisioning detects the package manager from the lockfile when the manifest doesn't name one, so the TypeScript and shell paths stop disagreeing on pnpm/yarn/bun projects (#870)** — `installWorktreeDeps` and `reinstallIfLockfileChanged` both resolved the manager as `(packageManager as keyof typeof PM_CONFIG) || "npm"`. That value comes from the worktree manifest, a snapshot taken at `sequant init`, so it is simply absent on a manifest-less or pre-init worktree — and the bare literal then assumed npm. Meanwhile `templates/scripts/new-feature.sh`, PM-aware since #847, detects live from the lockfile. On a pnpm project without a manifest the two provisioning paths therefore ran _different_ frozen installs for the same repo (`npm ci` vs `pnpm install --frozen-lockfile`), the dual-producer drift class of #833. Both sites now go through a new `resolvePackageManager(declared, root)` in `stacks.ts` — the module `new-feature.sh` already names as its source of truth — which keeps a declared manager and otherwise falls back to the same `detectPackageManagerSync` that #487 introduced for exactly this purpose. This is the continuation of #487's conversion pass, which converted `version-check.ts` and `bin/cli.ts` but never had `worktree-manager.ts` in its inventory. An own-property membership test also retires the unchecked cast: a declared value outside `PackageManager` (a hand-edited manifest, a future `"npm@10"` spelling) used to index `PM_CONFIG` to `undefined` and throw a `TypeError` on the next property access, and now routes to detection instead. The test is `hasOwnProperty` rather than `in` deliberately — `in` walks the prototype chain, so `"toString"`, `"constructor"`, and `"__proto__"` would pass it and be returned as package managers, reintroducing the same `.split(" ")` crash one layer down. Python managers (`pip`/`poetry`/`uv`) are `PM_CONFIG` keys, so a declared Python manager still wins over the JS-only detector. The same conversion covers `sequant update`'s three sites (`update.ts:113`/`:128`/`:296`), which carried the byte-identical spelling: because the manifest field is absent precisely on pre-1.3.0 installs — the population the `PM_RUN` backfill exists to serve — `update` was writing an npm `PM_RUN` token and running `npm install` in pnpm/yarn/bun projects. All three now read one manager resolved once from the manifest and the project's lockfile, so the rule has a single home rather than three copies. Manifest-less **npm** projects and lockfile-less projects are unaffected — detection returns `npm` there, exactly as the old literal did — and a manifest that _does_ record a manager still wins over the lockfile. Mutation-verified gate tests pin the manifest-absent case for pnpm and yarn at the worktree call sites, and for pnpm/yarn/bun across `update`'s backfill and first-run config paths. (#870)
...
- **`parseQaVerdict` choked on emoji-prefixed verdicts — genuine QA passes recorded as failures** — the QA verdict regex (`src/lib/workflow/phase-executor.ts`) only tolerated `**`/whitespace between `Verdict:` and the token, so a QA agent writing `Verdict: ✅ READY_FOR_MERGE` (the common form) parsed as `null` and the run was reported as _"QA completed without a parseable verdict"_ despite a real pass (live repro: `sequant run 687 --phases exec,qa`, 2026-06-01 — QA had posted READY_FOR_MERGE, applied `ready-for-review`, and opened PR #688). The gap between `Verdict:` and the token now tolerates any run of non-alphanumeric characters (emoji `✅`/`❌`/`⚠️`, asterisks, whitespace) via a negated ASCII class — ReDoS-safe and `no-misleading-character-class`-clean, matching `parseQaSummary`'s approach. This also hardens `sequant ready` (#683), whose #534 guard would otherwise misclassify an emoji-verdict QA pass as `NO_IMPLEMENTATION`. Regression tests added in `phase-executor.test.ts`. (#683)
