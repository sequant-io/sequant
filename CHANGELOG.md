...
- **`role:` model references and model-ladder rungs resolve against the `--agent` driver when `--agent` overrides `run.agent`.** They were resolved against `settings.run.agent` even when the run used a different driver. (#1150)
