...
- **`/qa` Sibling-site Scan prompt** — adds a conditional reviewer prompt to `/qa` SKILL.md §5 Risk Assessment that fires before the verdict on focused-AC PRs touching multi-pattern files. Before declaring AC met, the reviewer scans the same file/module for sibling code matching the bug's root cause; gaps are surfaced as expanded scope (only when trivial), follow-up issue suggestion, or `NEEDS_VERIFICATION`. The bullet explicitly says "don't widen scope mid-PR; file a follow-up issue instead" so it stays a safety net rather than a scope-creep incentive, and points at `feedback_qa_second_look.md` for the underlying principle. Mirrored across `.claude/skills/qa/SKILL.md`, `templates/skills/qa/SKILL.md`, and `skills/qa/SKILL.md`. Closes the gap exposed by `/fullsolve 564` where four sibling regexes in `pre-tool.sh` with the same root cause as the literal AC went unsurfaced through first-pass /qa, requiring a manual "any gaps?" nudge to file #570. (#573)
