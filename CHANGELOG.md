...
- **`sequant update --yes` / `-y` for non-interactive updates (#709)** — `update` was interactive-only: piping or running without a controlling TTY produced an unhandled `ExitPromptError` stack trace instead of a clean exit, and there was no flag to auto-apply. Added `-y, --yes` to apply updates without prompting, and a TTY guard on **both** prompt sites (first-run dev-URL setup and the apply-confirm) so that when a prompt is needed but stdin is not a terminal and `--yes` was not passed, `update` prints an actionable message naming `--yes` and exits non-zero — no more raw stack trace. `--yes` (auto-answer) stays distinct from `--force` (overwrite in-place customizations). Documented the automation path in the cheat-sheet and troubleshooting guide. (#709)
