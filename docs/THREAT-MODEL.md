...
| Behavioural injection eval with deterministic graders | `evals/qa-trust-boundary/graders/exfil-absent.md` and `.github/workflows/plugin-eval.yml` | deterministic | Manual dispatch, and one vector (issue body) today; PR-comment and tool-output vectors are tracked in #1024 |
