...
| Observability | Fixed by #860 AC-6: TTY heartbeat line, and now periodic non-TTY notices naming the wake time in both the quiet-mode heartbeat and the default-mode NonTTY renderer. | Trivial — the process exits and the state file says why and until when. |
