...
| D25 | Node 16 dogfood run on 2026-09-08: the opencode arm hit the OpenRouter credit wall ($10 account, $5.16 spent, $0.34/min for three issues) after exposing four defects (D-1 spec runs in the main checkout so the shim check fails; D-2 a failed spec triggers the quality loop; D-3 no /spec comment posted; D-4 provider credit reservation surfaces only as APIError); the owner stopped the run before the claude-code arm. Table recorded in `docs/investigations/multi-backend-2026-07.md` §"Dogfood 2026-09" with the comparison rows marked not run. Gate #997 presented with that table; decision pending. | run logs 16:01Z–16:17Z; opencode log "This request would exceed your available credits" | funding the arm (owner declined for now); fabricating comparison rows |
| D26 | Gate #997 **deferred** by the owner on 2026-09-08: opencode remains experimental; the dogfood re-runs after funding (OQ-27). D-1 and D-2 must be fixed before the re-run (they kill every attempt at spec); D-3/D-4 stay findings for the re-run to test. No accept-route docs node filed; no reject-route ACs filed yet. | #997 gate comment; dogfood table | accept (no verdicts reached); reject now (would file ACs without a funded re-run to confirm D-3/D-4) |
| D27 | Node 10's `fixture_commit` gate (#993 AC-4) records the *branch* commit a result was produced at; squash-merging (#1009 → `f94be343`) made all five recorded SHAs non-ancestors of `main`, so the gate fails on every full clone while CI passes only because a shallow checkout is treated as unevaluable. Fixed by repointing the five results at the squash commit, whose fixture content is byte-identical (small PR). Follow-up (needs its own row before filing): bind the gate to the fixture files' blob hashes, not a commit, so squash merges cannot invalidate a true provenance claim. | `npx vitest run __tests__/evals-fixture-commit.test.ts` on a full clone of `2efe2209`: 5 failed; CI green via the shallow-repo escape hatch | leaving the data as-is (main red for every developer); disabling the gate |
...
| OQ-25 | ~~Node 10 AC-6 budgets "P0.5 figure × 4" (= $2.84) but AC-1/2/3/5 mandate 9 arm-runs; PR #1009 spent $3.72 = $0.41 per arm-run, 42% under the Phase 0 rate. Amend AC-6 to a per-arm-run rate?~~ | resolved 2026-09-08 by owner: AC-6 amended to a per-arm-run rate ($0.71) + $25 issue cap; #993 body + node 10 updated; PR #1009 merges | amend to "per arm-run cost under the Phase 0 rate ($0.71), total recorded" and accept |
...
| OQ-27 | How is the dogfood re-run funded, and which fixes precede it? Options: OpenRouter top-up (~$40 for a 3-issue arm at $0.34/min) or `opencode auth login` with the Anthropic API key (bills the console account; needs the model switched to the `anthropic/` provider). Prerequisite fixes: D-1 (driver shim check vs spec cwd; `init` docs) and D-2 (orchestrator: failed spec must not dispatch the loop) — both need §7 rows before filing. | owner; before the #997 re-run | fund via `opencode auth login` (no reservation wall), fix D-1/D-2 as one small driver/orchestrator issue, then re-run all three issues on both arms |
