...
| D14 | Field finding 2026-09-10 (a downstream 2.11→2.14 sync): `computeTemplateChanges` walks the whole templates tree, so `sync --dry-run` and `update` map `templates/opencode/**` → `.claude/opencode/**` (unrendered `{{PHASE}}`, a path nothing reads) while `sync`'s `copyTemplates` never writes it — dry-run ≠ apply, and `serve`'s preflight nags "N files differ" on every pre-shim project. Filed as #1030 (node 09), **Blocked by #990** because both edit `templates.ts`/`sync.ts`; not added to wave 1. | The verbatim transcript in #1030; `templates.ts:194-225` (no skip list), `:600-665` (fixed dir list); `init.ts:145-175,300-330` (the only real shim producer). | Folding into #990 (scope growth on the largest node; the skill's no-expansion rule); running it in wave 1 (file collision with node 01). |
| D15 | #982 AC-3's third verify command (`grep -c "fullQa" src/lib/cli-flags.ts` ≥ 1) is unsatisfiable and was amended on the issue by the runner to `grep -c "fullQa" src/lib/workflow/types.ts` ≥ 2. | `cli-flags.ts` holds value-taking coercions only; the precedent boolean flags `--ready-gate`/`--escalate-effort`/`--auto-merge` have 0 hits there. QA's first pass flagged it `pause_for_human`. | Adding a no-op coercion to satisfy the grep (a tautology); leaving AC-3 partial and looping QA forever. |
| D16 | #990 AC-6 is scoped to `sync --dry-run`; the `update --dry-run` clause moves to #1030 (node 09). | `update`'s apply path never regenerates `AGENTS.md` nor re-links `scripts/dev` (writes rendered files only, `src/commands/update.ts`), so an `update --dry-run` that printed those decisions would itself be dry-run ≠ apply — the I-2 violation node 09 owns; `update.ts` is outside node 01's may-touch list. | Wiring the decisions into `update --dry-run` without an apply path behind them (a lying preview); growing node 01 into `update.ts`. |
| D17 | Field finding 2026-09-10 (wave-1 execution): exec phase agents that start the test suite as a `Monitor` or a backgrounded Bash command and then "wait for the notification" end the phase with the work uncommitted — under `sequant run` the end of the turn is the end of the phase and no notification is ever delivered. Filed and fixed as #1032 (PR #1034): `pre-tool.sh` blocks `Monitor`/`run_in_background` under `SEQUANT_ORCHESTRATOR`, `/exec` §3 states the foreground-with-`timeout` rule, and the #879 guard commits an `uncommitted` exec tree as `chore(#N): wip checkpoint …` before failing. Not a graph node — a repo defect the wave exposed. | Six stranded transcripts across three projects (ad-motion #226/#233, vid-gen bench #5/#6, sequant #933/#990) with zero task notifications delivered; #980 polled instead and idled into the MCP 30-min kill. | Skill text alone (model-dependent; the agents had the commit-cadence instruction and ignored it); raising the MCP timeout (does not address the turn-end semantics). |
| D18 | #1025 AC-2's verify command (`curl -sI <badge> \| head -1` → `HTTP/2 200`) was amended by the runner to `curl -sIL … \| grep -c "^HTTP/2 200"` → `1`. | The badge host answers `302` first and serves the SVG on the redirected URL; the first-line check could never pass. | Leaving AC-2 permanently PARTIALLY_MET. |
| D19 | Merge #1024's PR #1043 with a **merge commit**, not a squash. | Its two new `evals/results/*.json` records anchor `fixture_commit` = branch commit `9fb8340c`; a squash drops it from `main`'s ancestry and `__tests__/evals-fixture-commit.test.ts` fails on every full-history clone (the class #1018 and #1021 each repaired with a post-merge repoint). A merge commit keeps the anchor reachable with no follow-up. | Squash + repoint follow-up (a third instance of the same toil); anchoring the base commit (wrong provenance). |
| D20 | The wave-2 field findings are filed, not folded: #1044 (state reconciler marks an issue merged when any squash title mentions `(#N)`; the `status --cleanup` next-action it emits would remove a live worktree) and #1045 (`computeDriftFingerprint` duplicates the templates→`.claude` mapping outside `templateDestination`; retired-file limitation undocumented). | Both surfaced by running/QA-ing wave 2; neither is a wave node. | Fixing inside #1030's PR (scope growth on a node that already took three QA passes). |
