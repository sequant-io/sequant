...
The Claude Code plugin doesn't invoke `npx` directly, and it doesn't point at a launcher file path either. `${CLAUDE_PLUGIN_ROOT}` (with or without a `:-` default) resolves against the _open project_, not the plugin's own install directory, when Claude Code loads `.mcp.json` as a project config — which the sequant checkout's own root `.mcp.json` is, since it's dual-use (see the contributor note below). A file-path launcher can't work in both contexts, so the plugin's `.mcp.json` ships the launcher **inline**: `"command": "node", "args": ["-e", "<launcher source>", "sequant@<pin>"]`, with the full source of `scripts/mcp-launch.mjs` embedded as the `-e` argument (generated by `scripts/generate-mcp-launch-inline.mjs` and kept byte-for-byte in sync by a test). No `${...}` placeholder appears anywhere in `command`, `args`, or `env`.

The inlined launcher spawns `npx -y sequant@<pin> serve` from a fresh, isolated temp directory instead of the project directory (#1084). This matters because `npx` resolves packages by walking up from its launch cwd looking for a local install — if the open project has any `sequant` in its own `node_modules` (a stale, unused devDependency, for example), a plain `npx ... serve` run from the project directory would silently run that local copy instead of the pinned version, and a pre-`serve` version fails the MCP handshake with `CONNECTION_CLOSED`. See [Troubleshooting → CONNECTION_CLOSED](../troubleshooting.md#mcp-server-dies-with-connection_closed--a-local-sequant-shadows-the-pin). The launcher removes its temp directory when the server exits; if the launcher itself is killed outright (SIGKILL) that directory is left behind. Each launch directory holds a `launcher.pid` file, and the next launch sweeps any `sequant-mcp-launch-*` directory whose launcher is no longer running (a directory with no pid file is removed only when empty and older than an hour). On Windows the launch goes through `cmd.exe`, because Node refuses to spawn `npx.cmd` without a shell; the package spec is validated against a strict charset before that happens. The shipped inline copy is regenerated from `scripts/mcp-launch.mjs` with `npm run mcp-launch:inline`, which also runs as the first step of `prepare:marketplace`.

Since `npx` no longer runs from the project, the launcher needs the real project path to hand to `sequant serve` (which `chdir`s there before doing anything else, so every MCP tool and resource still operates on your project exactly as before). It gets that path from its own `process.cwd()` at startup — Claude Code always spawns the launcher with cwd set to the open project, the same fact that makes the shadowing possible in the first place — and forwards it to the child as `SEQUANT_PROJECT_DIR`. There's no placeholder to wire and nothing for you to set by hand.

**Windows residual risk:** the isolated launch directory comes from `os.tmpdir()`, which on Windows sits under the user profile (`%USERPROFILE%\AppData\Local\Temp`). A `sequant` package installed directly under `%USERPROFILE%\node_modules` — the same "home-stray" case `sequant doctor`'s shadow check already warns about — would still be an ancestor of that temp directory and could shadow the launch. This is accepted as a documented residual risk rather than fixed: it requires a local install directly in the user's home directory, which `doctor` flags, and no `npx` flag or launch-directory choice can fully rule out an ancestor match on Windows the way it can on POSIX systems where temp directories don't nest under `$HOME`.
