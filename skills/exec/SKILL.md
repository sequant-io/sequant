...
allowed-tools:
  ...
  - Bash(git log:*)
  - Bash(git worktree:*)
  # Worktree management (plugin paths)
  - Bash(${CLAUDE_PLUGIN_ROOT}/scripts/new-feature.sh:*)
  - Bash(${CLAUDE_PLUGIN_ROOT}/scripts/cleanup-worktree.sh:*)
  # GitHub CLI
  - Bash(gh issue view:*)
...
2. **Create worktree if needed:**
   - From the main repository directory, run: `${CLAUDE_PLUGIN_ROOT}/scripts/new-feature.sh <issue-number>`
   - This will:
...
4. **After implementation is complete:**
   ...
   - The worktree will be cleaned up after PR merge using `${CLAUDE_PLUGIN_ROOT}/scripts/cleanup-worktree.sh <branch-name>`
