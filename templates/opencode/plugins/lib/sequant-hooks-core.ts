/**
 * Sequant hook shim core for opencode (#996, #862 P1).
 *
 * Lives in `plugin/lib/` deliberately. opencode scans `.opencode/plugin/*.ts`
 * **non-recursively** and requires every export of a scanned file to be a
 * function — a module exporting a constant fails to load with
 * "Plugin export is not a function". Worse, that failure is logged at ERROR
 * and otherwise swallowed: exit 0, empty stderr, tools resolved. So the
 * constants and pure helpers below live one directory down, where they are
 * imported normally rather than scanned, and `../sequant-hooks.ts` exports
 * nothing but the plugin function itself.
 *
