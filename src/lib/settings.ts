...
/**
 * opencode-specific settings for the opencode agent driver (#862).
 */
export interface OpencodeSettings {
  /** Model to use, in opencode's `provider/model` form (e.g. "openrouter/anthropic/claude-sonnet-5") */
  model?: string;
  /** Agent variant to run the phase as (opencode `--variant`) */
  variant?: string;
  /** Extra CLI arguments passed to opencode */
  extraArgs?: string[];
  /**
   * Per-step reasoning-token budget (#992). opencode clamps completion at
   * ~32K tokens per step and `limit.output` does not raise it, so a reasoning
   * model can spend the whole step thinking and emit nothing. Injected as
   * `provider.<p>.models.<m>.options.reasoning.max_tokens` via
   * `OPENCODE_CONFIG_CONTENT`, which requires `model` to also be set.
   */
  reasoningMaxTokens?: number;
}

/**
 * Codex-specific settings for the codex agent driver (#497).
 */
export interface CodexSettings {
  /** Model to pass as `codex exec -m <model>` (e.g. "gpt-5-codex") */
  model?: string;
  /**
   * Sandbox policy for `codex exec -s <mode>`. Defaults to `workspace-write`:
   * `exec` itself defaults to read-only, which cannot write the worktree.
   *
   * Kept as a literal union rather than importing the Codex SDK's
   * `SandboxMode` so no reference to that devDependency survives into the
   * emitted `.d.ts` (#497 AC-8). `codex.test.ts` asserts the two stay
...
export interface RunSettings {
   ...
   * Default: "claude-code". Set to "aider" to use Aider CLI, or "opencode"
   * to use the opencode CLI (#862).
   */
  agent?: string;
  /**
  ...
  aider?: AiderSettings;
  /**
   * opencode-specific configuration. Only used when agent is "opencode" (#862).
   */
  opencode?: OpencodeSettings;
  /**
   * Codex-specific configuration. Only used when agent is "codex" (#497).
   */
  codex?: CodexSettings;
  /**
   ...
   *
   * Ordered rungs, cheapest first (e.g. `["sonnet", "opus", "fable"]`, or
   * role references: `["role:fast", "role:strong", "role:frontier"]`).
   * Absent by default — with no ladder configured the feature is fully off
   * and retried phases behave exactly as #914/#915 ship them.
   *
   * Escalation is routed on the deterministic no-progress signals only
   * (`LOOP_NO_DIFF`, `SAME_SHA_NO_PROGRESS`), never on repeated QA failure at
   * advancing SHAs — that pattern is divergence-suspect, and a stronger model
   * only rediscovers the contradiction more expensively. See
   * `workflow/model-ladder.ts`.
   *
   * Overridable per-invocation with `--model-ladder` (CLI > settings >
   * absent).
   */
  modelLadder?: string[];
  /**
...
/** Zod schema for OpencodeSettings (#862) */
export const OpencodeSettingsSchema = z.object({
  model: z.string().optional(),
  variant: z.string().optional(),
  extraArgs: z.array(z.string()).optional(),
  reasoningMaxTokens: z.number().positive().optional(),
});

/** Zod schema for CodexSettings (#497) */
...
export const RunSettingsSchema = z.object({
  ...
  /**
   * Model escalation ladder for capability-bound non-convergence (#971).
   * Ordered rungs, cheapest first. Absent by default (not `[]`) — an omitted
   * key must leave `ExecutionConfig` without a ladder key at all, so an
   * unconfigured run is byte-identical to pre-#971. Entries are plain strings
   * here; `role:<name>` references resolve against `run.modelRoles` in
   * `config-resolver.ts` (#975 AC-3 keeps raw model strings legal).
   */
...
/**
 * Validate opencode-specific settings (#862).
 * Throws on invalid types to catch config errors at load time.
 */
export function validateOpencodeSettings(
  opencode: unknown,
): OpencodeSettings | undefined {
  if (opencode == null) return undefined;
  if (typeof opencode !== "object" || Array.isArray(opencode)) {
    throw new Error("settings.run.opencode must be an object");
  }
  const obj = opencode as Record<string, unknown>;
  if (obj.model !== undefined && typeof obj.model !== "string") {
    throw new Error("settings.run.opencode.model must be a string");
  }
  if (obj.variant !== undefined && typeof obj.variant !== "string") {
    throw new Error("settings.run.opencode.variant must be a string");
  }
  if (obj.extraArgs !== undefined) {
    if (
      !Array.isArray(obj.extraArgs) ||
      ...
      throw new Error(
        "settings.run.opencode.extraArgs must be an array of strings",
      );
    }
  }
  if (obj.reasoningMaxTokens !== undefined) {
    if (
      typeof obj.reasoningMaxTokens !== "number" ||
      !Number.isFinite(obj.reasoningMaxTokens) ||
      obj.reasoningMaxTokens <= 0
    ) {
      throw new Error(
        "settings.run.opencode.reasoningMaxTokens must be a positive number",
      );
    }
  }
  return obj as unknown as OpencodeSettings;
}

/** Sandbox policies `codex exec -s` accepts (#497). */
...
/**
 * Validate codex-specific settings (#497).
 * Throws on invalid types to catch config errors at load time.
 */
export function validateCodexSettings(
  codex: unknown,
): CodexSettings | undefined {
  if (codex == null) return undefined;
  if (typeof codex !== "object" || Array.isArray(codex)) {
    throw new Error("settings.run.codex must be an object");
  }
  const obj = codex as Record<string, unknown>;
  if (obj.model !== undefined && typeof obj.model !== "string") {
    throw new Error("settings.run.codex.model must be a string");
  }
  if (
    obj.sandboxMode !== undefined &&
    !CODEX_SANDBOX_MODES.includes(obj.sandboxMode as never)
  ) {
    throw new Error(
  ...
  if (obj.extraArgs !== undefined) {
    if (
      ...
      throw new Error(
        "settings.run.codex.extraArgs must be an array of strings",
      );
    }
  }
  return obj as unknown as CodexSettings;
}
...
| \`agent\` | string | — | Agent driver for every phase: \`"claude-code"\` (default), \`"aider"\`, \`"opencode"\` or \`"codex"\` |
| \`phases\` | object | — | Per-phase overrides keyed by phase name (\`spec\`, \`exec\`, \`qa\`, …). Each entry takes \`agent\` (driver for that phase, overriding \`agent\` above; #1150), \`model\` and \`effort\` (\`low\`…\`max\`), which go to that phase's driver (#914). \`--models\`/\`--efforts\` override \`model\`/\`effort\` |
| \`modelLadder\` | string[] | — | Model escalation ladder, cheapest rung first (e.g. \`["sonnet","opus","fable"]\`, or \`role:\`-prefixed entries resolved via \`run.modelRoles\`). Absent = off: retried phases keep their configured model. When set, a retried phase whose prior attempt made **no progress** (\`LOOP_NO_DIFF\` / \`SAME_SHA_NO_PROGRESS\`) re-dispatches one rung up and stays there; repeated QA failure at *advancing* SHAs never escalates. Raises token spend — opt-in (#971) |
...
### \`run.opencode\` — opencode Settings (when agent="opencode")

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| \`model\` | string | — | Model in opencode's \`provider/model\` form (e.g. "openrouter/anthropic/claude-sonnet-5") |
| \`variant\` | string | — | Agent variant passed as \`--variant\` |
| \`extraArgs\` | string[] | — | Extra CLI arguments passed to opencode |
| \`reasoningMaxTokens\` | number | — | Per-step reasoning budget, injected as \`options.reasoning.max_tokens\`. Requires \`model\` (#992) |

### \`run.codex\` — Codex Settings (when agent="codex")

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| \`model\` | string | — | Model passed as \`codex exec -m\` (e.g. "gpt-5-codex") |
| \`sandboxMode\` | string | \`"workspace-write"\` | Sandbox policy: "read-only", "workspace-write", "danger-full-access". \`codex exec\` itself defaults to read-only, which cannot write the worktree (#497) |
| \`extraArgs\` | string[] | — | Extra CLI arguments passed to codex, inserted before the prompt |
