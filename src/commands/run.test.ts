...
describe("pre-PR rebase", () => {
  ...
  describe("PR creation", () => {
    describe("createPR", () => {
      ...
      // #605: --stacked plumbing
      describe("stacked mode (#605)", () => {
        function mockHappyPathToCreate(): void {
          // gh pr view fails (no existing PR)
          mockSpawnSync.mockReturnValueOnce({
            status: 1,
            stdout: Buffer.from(""),
          ...
          mockSpawnSync.mockReturnValueOnce({
            status: 0,
            stdout: Buffer.from("https://github.com/org/repo/pull/77\n"),
            stderr: Buffer.from(""),
            signal: null,
            pid: 0,
            output: [],
        ...
        it("passes --base <predecessor> to gh pr create when prBase is set", () => {
          mockHappyPathToCreate();
          ...
          createPR(
            ...
            undefined,
            {
              prBase: "feature/100-first",
              stackManifest: "Part of stack: #100 → #101 (this) → #102",
            },
          );

          // Third spawnSync call is gh pr create
          const ghCreateCall = mockSpawnSync.mock.calls[2];
          expect(ghCreateCall[0]).toBe("gh");
          const args = ghCreateCall[1] as string[];
          expect(args).toContain("--base");
          expect(args[args.indexOf("--base") + 1]).toBe("feature/100-first");
        });

        it("does not pass --base when prBase is undefined (final-in-stack)", () => {
          mockHappyPathToCreate();
          ...
          createPR(
            ...
            {
              prBase: undefined,
              stackManifest: "Part of stack: #100 → #101 → #102 (this)",
            },
          );
        ...
        it("embeds stack manifest in PR body when provided", () => {
          mockHappyPathToCreate();
          ...
          expect(body).toContain("Part of stack: #100 → #101 (this) → #102");
          // Manifest appears before the trailer
          expect(body.indexOf("Part of stack:")).toBeLessThan(
            body.indexOf("🤖 Generated by"),
          );
        });

        it("omits manifest from body when stackOptions is undefined (non-stacked)", () => {
          mockHappyPathToCreate();
          ...
          expect(body).not.toContain("Part of stack:");
        });
      });
    });
  });
