---
description: Run the sequant testgen phase for a GitHub issue
---

Load the `testgen` skill with the skill tool, then execute it against the
arguments below.

**This is not optional and not a summary request.** The skill file is sequant's
methodology for this phase; a response written without it is not a `testgen`
result, however plausible it reads.

1. Call the skill tool with `{"name": "testgen"}`.
